...
/**
 * The five tester seats: three operators and two coaches.
 *
 * Seats, not people. Brian decides who sits in each one at hand-out time, so
 * nothing here — and nothing in a generated checklist — names anybody. What a
 * seat does carry is the **capability it needs**, because a checklist that
 * tells its reader to press something their account cannot press is a broken
 * checklist: `needs` is what Brian matches a person against.
 *
 * The two coach seats are not spare operators. A coach account genuinely holds
 * no role management, which is the only way to walk
 * `WF-role-management-authority`, and the register is taken on a phone rather
 * than at a desk. The player's and the recruit's own pages need no account at
 * all — they open from a link — so they sit with the second coach rather than
 * inventing a sixth seat for them.
 */
export const TESTERS = Object.freeze({
  tester1: {
    name: "Tester 1",
    needs: "An operator account holding the core four roles",
    file: "tester-1.md",
  },
  tester2: {
    name: "Tester 2",
    needs: "An operator account holding the core four roles, including the President's office",
    file: "tester-2.md",
  },
  tester3: {
    name: "Tester 3",
    needs: "An operator account with administration — operators, roles and messaging",
    file: "tester-3.md",
  },
  tester4: {
    name: "Tester 4",
    needs: "A coach account, used on a phone at the pitch",
    file: "tester-4.md",
  },
  tester5: {
    name: "Tester 5",
    needs: "A coach account, plus the player and recruit links handed out with this list",
    file: "tester-5.md",
  },
});
...
export const WORKFLOWS = Object.freeze([
  ...
  wf(
    ...
    "President, General Manager or IT Officer",
    "tester2",
    ["/operate/admin/roles/{role.kit_manager}", "/operate/admin/operators/{operator.spare}"],
    ["seat.held"],
  ...
  wf(
    ...
    "President, General Manager or IT Officer",
    "tester3",
    ["/operate/admin/operators/{operator.spare}"],
  ...
  wf(
    ...
    "tester3",
    ["/operate/admin/operators/{operator.spare}", "/operate/admin/roles/{role.kit_manager}"],
    ["audit.row"],
  ...
  wf(
    ...
    "Any operator reading; an administrator changing",
    "tester3",
...
${workflows
  .map((workflow) => {
    ...
    const tester = workflow.tester
      ? `<span class="chip tester">${esc(TESTERS[workflow.tester].name)}</span>`
      : "";
