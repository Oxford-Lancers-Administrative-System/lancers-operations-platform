...
const LEGACY_EPOCH_EVENT_TYPES = [
  "lead-epoch-opened",
...
const REVIEW_INVOCATION_EVENT_TYPES = [
...
/** Stable fallback for journals whose approved decomposition predates planned epochs. */
function compatibilityEpochs(state) {
  const remaining = Object.values(state.packages).filter((pkg) => pkg.status !== "removed");
  const ordered = [];
  const placed = new Set();
  while (remaining.length > 0) {
    const index = remaining.findIndex((pkg) =>
      (pkg.depends_on ?? []).every((dependency) => placed.has(dependency)),
    );
  ...
  for (let index = 0; index < ordered.length; index += 2) {
    epochs.push({ id: `E-${epochs.length + 1}`, package_ids: ordered.slice(index, index + 2) });
  }
  return epochs;
}

/** The issue groups approved with the plan, or a deterministic legacy projection. */
export function executionEpochs(state) {
  const planned = state.decomposition?.execution_epochs;
  return Array.isArray(planned) && planned.length > 0 ? planned : compatibilityEpochs(state);
}

function executionEpochComplete(state, epoch) {
  return epoch.package_ids.every((id) => {
    const pkg = state.packages[id];
    return pkg?.status === "removed" || (pkg?.status === "merged" && state.reclaimed.includes(id));
  });
}

/** The one planned issue group this run is finishing before it returns. */
export function currentExecutionEpoch(state) {
  const completed = new Set(state.completedEpochs);
  const epochs = executionEpochs(state);
  const index = epochs.findIndex((epoch) => !completed.has(epoch.id));
  if (index < 0) return null;
  const epoch = epochs[index];
  const finished = epoch.package_ids.filter((id) => {
    const pkg = state.packages[id];
    return pkg?.status === "removed" || (pkg?.status === "merged" && state.reclaimed.includes(id));
  });
  return {
    id: epoch.id,
    index,
    package_ids: [...epoch.package_ids],
    issue_ids: epoch.package_ids.map((id) => state.packages[id]?.linear_issue_id ?? null),
    completed_packages: finished,
    complete: executionEpochComplete(state, epoch),
  };
}
...
export function validateEvent(event, state) {
  ...
  switch (event.type) {
    ...
    case "plan-recorded": {
      ...
      for (const epochId of state.completedEpochs) {
        const previous = executionEpochs(state).find((epoch) => epoch.id === epochId);
        const revised = event.decomposition?.execution_epochs?.find(
          (epoch) => epoch.id === epochId,
        );
        if (
          !previous ||
          JSON.stringify(revised?.package_ids) !== JSON.stringify(previous.package_ids)
        ) {
          errors.push(
            `Completed execution epoch ${epochId} is immutable; replanning may change only later groups.`,
          );
        }
      }
    ...
    case "lead-epoch-closed":
      // Historical journals keep replaying, but these events no longer define
      // or constrain runtime behavior.
      break;
    default:
      break;
  }
...
export function reduce(events) {
  ...
  for (const [index, event] of events.entries()) {
    switch (event.type) {
      ...
      case "checkpoint": {
        const epoch = currentExecutionEpoch(state);
        if (epoch?.complete && !state.completedEpochs.includes(epoch.id)) {
          state.completedEpochs.push(epoch.id);
        }
...
function ordinaryActions(state) {
  ...
  const epoch = state.planApproved ? currentExecutionEpoch(state) : null;
  const epochPackages = new Set(epoch?.package_ids ?? []);
  if (epoch?.complete) {
    actions.push({
      action: "checkpoint-and-stop",
      detail: `${epoch.id} is complete. Record a checkpoint and stop this run before starting the next planned issue group.`,
    });
  }
  for (const pkg of packages) {
    ...
    if (epoch && (epoch.complete || !epochPackages.has(pkg.id))) continue;
  ...
  if (!epoch && allMerged && !smoke && smokes.length === 0 && !walkerInvocation) {
    actions.push({
  ...
  if (!epoch && allMerged && smoke?.result === "blocked" && smokes.length === 1) {
    actions.push({
  ...
  if (!epoch && allMerged && smokes.length >= 2 && smokes.at(-1)?.result === "blocked") {
    actions.push({
  ...
  if (!epoch && allMerged && finalMissionSmokeClear(state)) {
    if (!state.closeout) {
      actions.push({
        action: "closeout",
        detail:
