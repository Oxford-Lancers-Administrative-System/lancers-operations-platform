...
select
  ...
  (select count(*) from public.people
    where (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) like 'PILOT-LAN-78%') as scenario_people,
  ...
  -- than assumed.
  (select count(*) from public.people
    where (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) is null or (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) not like 'PILOT-LAN-78%') as people_that_stay,
...
begin
  ...
  select count(*) into offending
   ...
   where season_membership_id in (
     select id from public.season_memberships
      where person_id in (select id from public.people where (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) like 'PILOT-LAN-78%'));
  if offending > 0 then
    raise exception
  ...
  -- Display aliases excluded: since LAN-182 the alias flagged as a person's
  -- display name is what carries this scenario's sentinel, so every scenario
  -- person has exactly one and it is not a foreign row. Any OTHER alias is
  -- somebody's record and still stops this script.
  select count(*) into offending
    from public.person_aliases
   where not is_display_name
     and person_id in (select id from public.people where (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) like 'PILOT-LAN-78%');
  if offending > 0 then
    raise exception
  ...
  select count(*) into offending
   ...
   where actor_person_id in (select id from public.people where (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) like 'PILOT-LAN-78%');
  if offending > 0 then
    raise exception
...
delete from public.people
 where id = '00780078-0078-4078-8078-000000000001'
   and (select da.alias from public.person_aliases da where da.person_id = people.id and da.is_display_name limit 1) like 'PILOT-LAN-78%';
