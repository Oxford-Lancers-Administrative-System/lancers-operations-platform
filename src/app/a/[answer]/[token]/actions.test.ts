...
// `answerSegmentOf` stays real: it is a pure parse of the token the action was
// given, and it is what decides the `/a/yes/` or `/a/no/` path every refusal
// below redirects back to (LAN-343). Mocking it would mean asserting against a
// path this test made up rather than the one the token names.
...
vi.mock("@/lib/services/player-home", async (importOriginal) => {
  ...
  return { ...actual, answerEventQuestionsIn: vi.fn(), readInvitationOwnerIn: vi.fn() };
});
...
import { answerEventQuestionsIn, readInvitationOwnerIn } from "@/lib/services/player-home";
import { openEventsPage, submitAnswer } from "./actions";
import { ERROR_PARAM } from "./params";
import { BUSY_ERROR } from "./presentation";

// A real answer token's exact shape — `y`/`n`, the invitation's uuid, then a
// 43-character nonce. LAN-343 made the shape load-bearing here as well as in
// the resolver: `answerSegmentOf` parses it to decide which of `/a/yes/` and
// `/a/no/` a refusal returns to, so a fixture one character short would send
// every assertion below to the wrong path.
const TOKEN = "y.11111111-1111-1111-1111-111111111111.abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLM0123";
...
describe("a successful answer", () => {
  it("ends on the durable page, focused on the invitation just answered", async () => {
    ...
    expect(target).toBe(
      `/events/${encodeURIComponent("durable-token-plaintext-000000000000000000000")}?open=00000000-0000-4000-8000-000000000002`,
    );
  });
...
describe("a closed answer link", () => {
  ...
  it("refuses an anonymous injected token without exposing a distinct outcome", async () => {
    ...
    expect(target).toBe(`/a/no/${encodeURIComponent(injected)}`);
...
/**
 * "See all your events." — LAN-343. The page this reaches used to be named in
 * prose ("check the most recent message from the club for a link to your own
 * page") and no message contained one. It needs a durable credential, whose
 * plaintext cannot be recovered, so this action mints one behind the click —
 * and the person it mints for comes from this link's own answer token, never
 * from the form.
 */
describe("openEventsPage", () => {
  const OWNER = {
    personId: "00000000-0000-4000-8000-000000000003",
    seasonId: "00000000-0000-4000-8000-000000000004",
    capacity: "player",
  ...
  function validAnswerToken(): void {
  ...
  it("mints a durable credential for the person the token resolves to", async () => {
    validAnswerToken();
    vi.mocked(readInvitationOwnerIn).mockResolvedValue(OWNER);

    const target = await redirectFrom(() => openEventsPage(formFor()));
    ...
    expect(target).toBe(
      `/events/${encodeURIComponent("durable-token-plaintext-000000000000000000000")}`,
    );
  });

  it("refuses a recruit — REQ-recruit-sees-public-only, there is no events page for them", async () => {
    validAnswerToken();
    vi.mocked(readInvitationOwnerIn).mockResolvedValue({ ...OWNER, capacity: "recruit" });
  ...
  it("refuses an anonymous injected token without exposing a distinct outcome", async () => {
    ...
    const target = await redirectFrom(() => openEventsPage(form));

    expect(resolveAnswerTokenIn).toHaveBeenCalledWith(expect.anything(), injected);
    expect(issuePersonTokenIn).not.toHaveBeenCalled();
    expect(target).toBe(`/a/no/${encodeURIComponent(injected)}`);
    expect(target).not.toContain("unknown");
  });
