...
function homeUrl(token: string, invitationId: string): string {
  return `/events/${encodeURIComponent(token)}?open=${encodeURIComponent(invitationId)}`;
}
...
function plainHomeUrl(token: string): string {
  return `/events/${encodeURIComponent(token)}`;
}
...
export async function changeToYes(form: FormData): Promise<void> {
  ...
  if (await throttled(token)) await refuse(`/events/${encodedToken}`, startedAt);
  ...
  try {
    ...
    await refuse(`/events/${encodedToken}`, startedAt);
  }
