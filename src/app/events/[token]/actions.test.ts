...
describe("changeToYes", () => {
  it("re-resolves the token and records Yes against the resolved person, not the form", async () => {
    ...
    expect(target).toBe(
      `/events/${encodeURIComponent(TOKEN)}?open=${encodeURIComponent(INVITATION_ID)}`,
    );
  });
  ...
  it("refuses uniformly when the durable token no longer resolves", async () => {
    ...
    expect(target).toBe(`/events/${encodeURIComponent(TOKEN)}`);
  });
  ...
  it("refuses an anonymous injected token without exposing a distinct outcome", async () => {
    ...
    // `null` is the events page's own purpose (LAN-343): this route resolves
    // that credential and nothing else, so a token minted for the onboarding
    // form or the opt-out is as unknown here as an injected string.
    expect(resolvePersonTokenIn).toHaveBeenCalledWith(expect.anything(), injected, null);
    expect(target).toBe(`/events/${encodeURIComponent(injected)}`);
    expect(recordPlayerHomeAnswerIn).not.toHaveBeenCalled();
  });
...
describe("submitNo", () => {
  ...
  it("returns to the same panel with a recoverable error on a blank reason, not the uniform refusal", async () => {
    ...
    expect(target).toBe(
      `/events/${encodeURIComponent(TOKEN)}?open=${encodeURIComponent(INVITATION_ID)}&reasonError=1`,
    );
  });
