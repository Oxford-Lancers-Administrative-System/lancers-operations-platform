...
describe("a well-formed link renders one button and spends nothing", () => {
  it("shows the button and never creates a Supabase client", async () => {
    await open({ token_hash: TOKEN, type: "recovery" });

    expect(screen.getByRole("button", { name: "Reset your password" })).toBeVisible();
  ...
  it("posts the token in hidden fields to the exchange route", async () => {
    ...
    const form = screen.getByTestId("recovery-confirm") as HTMLFormElement;
    expect(form.getAttribute("method")).toBe("post");
    expect(form.getAttribute("action")).toBe("/auth/recovery/exchange");
    expect(Object.fromEntries(new FormData(form))).toEqual({ token_hash: TOKEN, type: "recovery" });
  });
...
describe("anything else goes to /reset-password without contacting the auth server", () => {
  it.each([
    ["no query at all", {}],
    ["no token", { type: "recovery" }],
    ["no type", { token_hash: TOKEN }],
    ["the wrong type", { token_hash: TOKEN, type: "invite" }],
    ["a malformed token", { token_hash: "not-a-token", type: "recovery" }],
    ["a repeated token", { token_hash: [TOKEN, TOKEN], type: "recovery" }],
    ["an injected token", { token_hash: "abc' or '1'='1", type: "recovery" }],
  ])("%s", async (_name, query) => {
    await expect(open(query)).rejects.toThrow("REDIRECT:/reset-password");
    expect(verifyOtp).not.toHaveBeenCalled();
  });
});
...
describe("a scanner's GET, then the person's click", () => {
  it("leaves the token for the POST, which exchanges it and lands on /reset-password", async () => {
    ...
    const form = screen.getAllByTestId("recovery-confirm")[0] as HTMLFormElement;
    const body = new URLSearchParams(
      [...new FormData(form)].map(([key, value]) => [key, String(value)]),
    );
    const response = await POST(
      new NextRequest(new URL("/auth/recovery/exchange", "http://0.0.0.0:8080"), {
        method: "POST",
    ...
    expect(verifyOtp).toHaveBeenCalledExactlyOnceWith({ type: "recovery", token_hash: TOKEN });
