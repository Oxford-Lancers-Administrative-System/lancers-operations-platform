 ...
 * `src/app/events/[token]/actions.test.ts`'s own pattern for the sibling durable
...
describe("agreeDocument", () => {
  ...
  it("returns to the same document with an error and resolves nothing when the box is unticked", async () => {
    ...
    expect(target).toBe(
      `/onboarding/${encodeURIComponent(TOKEN)}?step=code_of_conduct&agreeError=1`,
    );
  });
  ...
  it("treats an already-agreed resubmission as done rather than a failure", async () => {
    ...
    expect(target).toBe(`/onboarding/${encodeURIComponent(TOKEN)}?step=code_of_conduct`);
  });
...
describe("submitTrustStep", () => {
  ...
  it("advances to the literal next step regardless of whether this one was just claimed", async () => {
    ...
    expect(target).toBe(`/onboarding/${encodeURIComponent(TOKEN)}?step=hudl`);
  });
  ...
  it("finishes the sequence after the last step", async () => {
    ...
    expect(target).toBe(`/onboarding/${encodeURIComponent(TOKEN)}?step=done`);
  });
