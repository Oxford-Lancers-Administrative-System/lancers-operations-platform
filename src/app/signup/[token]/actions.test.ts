...
function values(overrides: Partial<SignupFieldValues> = {}): SignupFieldValues {
  return {
    ...
    familyName: "Recruit",
    mobile: "07700 900556",
    collegeEmail: "lan202.token@balliol.ox.ac.uk",
    email: "",
    knownAs: "",
    college: "",
    matriculationYear: "",
...
async function mintPersonAndToken(): Promise<{ personId: string; token: string }> {
  ...
  // LAN-343: this route accepts the `recruit_signup` credential and nothing
  // else, which is the recruit gate `/me/join/[token]` never had — it used to
  // take any durable token, including the one that opens somebody's events
  // page.
  const issued = await withTransaction((tx) =>
    issuePersonTokenIn(tx, person.rows[0].id, seasonId, { purpose: "recruit_signup" }),
  );
