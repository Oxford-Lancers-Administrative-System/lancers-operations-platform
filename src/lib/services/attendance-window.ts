 ...
 * The two answers differ for several hours on the day, and that is the point.
 *
 * Whether a **save** is accepted is `./attendance.ts`'s. It gates on the stored
 * status being `approved` — the check constraint this branch's migration leaves
 * in place of `attendance_records_require_an_occurred_event` — and then on this
 * file, through the same function the board uses.
 */
