...
export async function dispatchRecruitmentCycleJob(
  ...
  const claim = await withTransaction(async (tx): Promise<CycleOutcome> => {
    ...
    // LAN-348, Brian's date decision: the four recruit-cycle templates anchor
    // on the day this person's record was opened, because Meta's classifier
    // requires a date in every Utility body and a recruit-cycle message has no
    // event to supply one. `opened_on` is the day they were added as a recruit,
    // formatted the way `when_label` is on the event path.
    ...
    return {
      ...
      message: {
        ...
        // LAN-348: `whenLabel` carries the day this person was added as a
        // recruit — the date slot every recruit-cycle template renders. The
        // subject beside it is fixed per kind in `templates.ts`. `eventName`
...
export async function dispatchOnboardingWelcomeJob(
  ...
  const claim = await withTransaction(async (tx): Promise<WelcomeOutcome> => {
    ...
    // LAN-348: `opened_on` is the day this person was added to onboarding —
    // the date slot the welcome and chase templates render, for the same
    // classifier rule the recruit cycle satisfies with its own opened-on date.
    ...
    return {
      ...
      message: {
        ...
        recipient,
        inviteeName: givenName,
        // LAN-348: the day this person was added to onboarding. `eventName`
  ...
  const claim = await withTransaction(async (tx): Promise<ChaseOutcome> => {
    ...
    return {
      ...
      message: {
        ...
        inviteeName: givenName,
        eventName: "",
        // LAN-348: the day this person was added to onboarding.
        whenLabel: membership.rows[0].opened_on,
        rsvpUrl: "",
        formUrl,
      },
