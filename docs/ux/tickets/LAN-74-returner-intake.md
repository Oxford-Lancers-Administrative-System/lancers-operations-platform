...
| ------ | -------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UX-10  | "Given name" / "Family name"                             | **"First name" / "Last name"**, first before last    | The frozen model's vocabulary was appearing on an operator's screen. The columns keep `given_name` / `family_name`; only the labels changed                       |
...
| Keyboard focus, labels, status meaning, error association, touch targets | **Partly evidenced.** Every field has a real `<label>`; validation focuses the first invalid control and associates its message; actions carry a 44px minimum. Not evidenced by an automated accessibility audit — there is none in this repository |
