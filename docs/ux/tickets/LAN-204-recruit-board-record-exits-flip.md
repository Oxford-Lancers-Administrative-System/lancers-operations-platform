...
### src/app/operate/destinations.ts — RECRUITMENT entry
...
### src/app/operate/recruitment/board-actions.ts — module header
...
### src/app/operate/recruitment/[prospectId]/record-view.tsx — RecruitmentRecordView, consent gate
...
### src/app/operate/recruitment/[prospectId]/record-view.tsx — headline strip
...
### src/app/operate/recruitment/[prospectId]/record-view.tsx — lower bands layout
...
### src/app/operate/recruitment/board-columns.ts — consent column label
...
### src/app/operate/board-filter-controls.tsx — file header

> LAN-204's own correction: Brian, 2026-09-02 — "The filters here are weird
> because the UI elements are completely different than what's on the
> roster page… it reinvented the shit." The roster keeps importing from
> here too, unchanged in appearance or behaviour; only where the code lives
> has moved.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/lib/services/messaging-consent.ts — mayReceiveWelcomeContactIn

> LAN-204's own consent deadlock (Brian, 2026-09-02: "If consent is not
> given, sending the personal questionnaire is how we get it… deadlocked"). The
> recruitment cycle's welcome track is the message that _carries the
> link to the sign-up form_, so it is the one send allowed to establish
...
> {@link hasGrantedSeasonMessagingConsentIn} exactly as shipped.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/lib/services/onboarding-welcome.ts — module header
...
> `notification_job_type` value nothing else in this codebase's _player-side_
...
### src/lib/services/recruitment-cycle.ts — declareRecruitmentCycleJobsIn
...
>    _via the sign-up form_ (`source: 'qr_self_entry'`), not merely `granted`
>    by any door.
...
### src/lib/services/recruitment-prospect/status.ts — updateRecruitmentProspectStatusIn
