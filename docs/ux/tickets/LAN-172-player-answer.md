...
### src/app/a/[token]/auto-submit.tsx — module header (Q-11/Q-30/OWNER-LAN172-17/-22, rounds 6-8).

> ## Round 6 shipped this unconditionally on mount, and that broke the gate
>
> The gate cookie `actions.ts` checks is set on _every_ GET to `/a/[token]`,
...
> that ruled out guessing _who_ a visitor is from a string it sends. This is
> direct evidence of _what actually happened_ in this browser, and it is
...
> a click that only _focuses_ the reason box, or opens a question's Select,
...
### src/app/a/[token]/presentation.ts — `confirmLabel()` (OWNER-LAN172-10).
...
### src/app/a/[token]/presentation.ts — `NO_HEADING` (OWNER-LAN172-13, overriding LAN-172-c2).

> W2's No-path section, read in full per the Mission Lead's instruction — this
> is the spec's own words for the page the player lands on immediately after
> tapping No in WhatsApp, describing their own stated choice and the reason
> field's honest current value, not a claim about which HTTP request has
> completed. Q-11 is unchanged by this: the GET underneath this heading still
> writes nothing; only the page's own form submission (`submitAnswer`) records
> anything, and does so whether or not the reason field was ever touched.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/a/[token]/presentation.ts — `QUESTIONS_HEADING` section banner (OWNER-LAN172-12/13).

> W2 line 61 (the approved landing-page table): the Yes landing "asks
> applicable event questions"; the No-path section: "the reason field
...
### src/app/me/[token]/presentation.ts — module header (LAN-172, W2-05/06, Q-22/Q-23).
...
answers — still to come` — ... while keeping every visual choice (buttons,
> chips, spacing, type scale) in the application's own idiom per Q-23.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — page-heading section banner (W2.html:956).

> Brian, 2026-08-26: put the player's own name above it too, so they know
> they are on the right page. No other personal detail.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — `pageHeading()` (OWNER-LAN172-14).

> `outstandingCount` (Q-26) counts only unanswered invitations, on purpose,
> and stays that way — this finding does not touch its horizon scoping.
> What changes is the heading's own claim: "nothing else needs an answer" is
> false whenever `Follow-up needed` is non-empty, so a second, honest
> heading covers that case instead of silently reusing the empty-queue one.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — `CHANGE_TO_NO` (OWNER-LAN172-11).

> `REQ-emphasis-points-at-yes` governs the control's fill/weight, not its
> wording — this control stays unfilled exactly as before, and a neutral
> label on an already-unfilled control is strictly less directional toward
> No than naming it, never more. `CHANGE_TO_YES` keeps its filled treatment
> and its own wording untouched; only this secondary control's copy changes,
> converging on the wording `PLANS_CHANGED` already uses ("change your
> answer") rather than inventing a second pattern.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — `NEXT_CHIP`/`AWAITING_ANSWER_CHIP`.

> The type chip stays alongside it: dropping which kind of event this is
> would lose real information the mockup's single-type demo never had to
> disambiguate.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — `SAVE_REASON` (OWNER-LAN172-09).

> Renamed from "Give a reason and continue" — there is nothing left to
> "continue" to once the No already stands (Q-22, `REQ-no-reason-given`);
> this action only ever saves an optional reason against an answer already
> recorded.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/app/me/[token]/presentation.ts — `QUESTIONS_RECORDED` (OWNER-LAN172-08).

> Shown in place of the questions form once nothing required remains
> outstanding, so a successful save reads as recorded rather than
> re-showing the identical form.
