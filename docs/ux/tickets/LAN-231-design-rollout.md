...
### src/components/link-opened-beacon.tsx — module header (LAN-269). Destination: `docs/ux/design-system.md` intake record.
...
### src/components/phone-field.tsx — module header (LAN-211, Brian 2026-09-01).
...
>
> LAN-211 is explicit that this is a standard rather than a one-off — "any
> ticket that has to deal with validation of a number needs to have the
> country code as one box and then separate it as a phone number".
>
> `DateField` in `./field.tsx` already established the hidden-input shape.
...
> hidden input means every one of them keeps working unchanged.
>
> The joined value carries an explicit `+` and calling code, which is the one
> form `toE164` never has to make a judgement about. That is what makes the
> country explicit rather than inferred: a fresher at a stand no longer
> carries the burden of writing the country code correctly, and a malformed
> number is caught at the source instead of failing silently at the Meta send.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/components/step-trail.tsx — module header (LAN-225, player-surfaces addendum).
...
> which the ragged wrap did not.

Relocated from a source comment by LAN-300; the source keeps a one-line pointer.

### src/theme.ts — module header, contrast table (LAN-225).
...
### src/theme.ts — module header, "What is decided here, and by whom" (LAN-225).
...
### src/components/public-shell.tsx — module header (LAN-225, brief §2).
...
> pages with none.
>
> `stack` is for a public page that is genuinely several sections — the
> player's own invitations, the questionnaire's steps — where one card around
> all of them would be a card around a whole page and the sections inside it
> would be cards inside a card. `wide` is `stack` because a calendar was
> never a panel.
