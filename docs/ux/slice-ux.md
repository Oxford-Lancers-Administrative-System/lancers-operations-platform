...
10. **Occurrence:** an approved event has occurred once its scheduled date has passed and it was not cancelled. **Nobody asserts it** — LAN-151 (D30) retired the decision and both of its screens, so time is exactly what determines occurrence and no surface offers Mark occurred, Mark not held, Confirm what happened or Correct this to not held. What replaced the assertion is the register itself: whether it has been saved is the record of whether the session was assessed (D71–D74).
11. **Attendance:** an approved event accepts Present, Absent, Late or Excused from the moment its register opens — about six hours before it starts (D71) — and the register never closes afterwards (D72). A draft or a cancelled event never opens one. Every recorder uses the LAN-80 model. RSVP remains independent; mismatches remain visible. Minimal walk-up identity is flagged for later reconciliation.
...
> **Amendment to D62 / D73 / D74 — response progress by capacity (Brian, 2026-09-22, on Stewart's request; LAN-420).**
>
> Those three decisions made the event page's headline three raw pairs — Invited, Said yes, Showed — with "no percentages, no judgment". Stewart's email "OPS EVENTS UPDATES" of 2026-09-22, change 2, following the State of the App call the same day, asks for the opposite at the top of the page: "the most important information is how many out of the total are saying yes… maybe there's a progress bar", and Clint on the call for "separating how many have RSVPd out of how many are invited by role". Brian accepted it.
>
> So **a percentage and a coloured bar are now part of the headline**, on the operator event page and on the public Event info link page alike. The top of each is one block per capacity present in the audience, in the order **Recruits, Players, Coaches, Committee**; each shows `yes / invited`, `no`, and a response bar of `(yes + no) / invited`, red below 50 %, orange to below 75 %, green at 75 % and above. A capacity with nobody invited shows no block, so a non-recruitment event with no recruits shows Players and Coaches only. A person invited under two capacities counts once, under the first of Recruits, Coaches, Players, Committee. The block counts **invitations**, so a late-added invitee raises the denominator. Labels, values and states only; nothing explains the bar in words.
>
> **What the three decisions still govern is Showed**, which keeps its exact `— / 37` and `0 / 37` shape — D74's distinction between an unsaved register and an empty one is untouched — wherever it still appears: the operator's list of events, the register itself, and the public Event info link page. The Invited / Said yes / Showed row is replaced by the blocks above; the whole-event totals are the sum of the blocks and are not repeated.
>
> **Correction, Brian's visual review of the built screen, 2026-09-22.** Two things changed. First: "Remove the Showed / Invited card entirely. The register panel below it stays; attendance is still recorded there." Stewart had asked for Showed to move below Audience and Distribution; seeing it there Brian took it off the operator event page altogether. The register panel keeps that position and is where attendance is recorded. Second: "'No 6' is meaningless as a line. Make it a labelled metric like the one above it: value `6`, label **Said no**." So the No figure in every block takes the Said yes / Invited pair's own shape — the number first, at the same weight, its label under it — on both pages.
>
> **Correction, Brian's walk of 573bb9d4, 2026-09-23 — the final shape.** Two more things changed, and this is the shape that shipped. First, **Showed goes from the public Event info link page too**, so the figure is on neither participation surface; it keeps its D74 shape on the operator's list of events and on the register, which are the two places it is acted on. Second, **one value line and a three-segment bar in place of two stacked metrics and a colour gate**. The value reads `16 yes · 6 no / 39` under the label **Said yes · Said no / Invited** — one sentence about one population, whose denominator belongs to both halves, rather than two metrics of equal weight reading as two separate facts. The bar is yes in the theme's success green from the left, no in the error red from the right, and the people who have not answered as the pale track between them, every width proportional to invited: "so one no in ten is a red tenth on the right and the gap is the chase." **The red / orange / green response-rate gate is dropped** with its 50 % and 75 % thresholds: the gap carries that information, and the threshold disagreed with the numbers at the edges, turning an event where everybody had answered and half said no green. Both surfaces render the same component. Everything else in this amendment stands.
...
| ~~UX-70~~ | ~~Confirm what happened~~ — **retired by LAN-151 (D30)** | —                                 | —                                                  | LAN-151          | The wireframes describe a decision nobody makes. The event page shows the register instead.                                                                                                                                    |
| UX-71     | Attendance is not available yet                          | `/operate/events/[id]/attendance` | Authorized attendance operator                     | LAN-80, LAN-151  | [`desktop`](wireframes/UX-71-attendance-locked-desktop.svg) / [`375px phone`](wireframes/UX-71-attendance-locked-phone.svg) — the wireframes show it waiting on an assertion; it now waits on the approval, or on D71's buffer |
...
| ~~UX-75~~ | ~~Event marked not held~~ — **retired by LAN-151 (D30)** | —                                 | —                                                  | LAN-151          | `not_held` left `public.event_status`; a cancellation is the only way an evening does not happen.                                                                                                                              |
...
| UX-90     | Attendance is not open                                   | `/operate/events/[id]/attendance` | Head Coach, OC or DC                               | LAN-110, LAN-151 | [`desktop`](wireframes/UX-90-coach-occurrence-locked-desktop.svg) / [`375px phone`](wireframes/UX-90-coach-occurrence-locked-phone.svg) — same correction as UX-71: the clock lifts it, not a person                           |
