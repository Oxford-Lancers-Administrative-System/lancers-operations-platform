...
>
> **What the three decisions still govern is Showed**, which keeps its exact `— / 37` and `0 / 37` shape (D74's distinction between an unsaved register and an empty one is untouched) and moves, with the register summary, **below Audience and Distribution**. The Invited / Said yes / Showed row is replaced by the blocks above; the whole-event totals are the sum of the blocks and are not repeated.
