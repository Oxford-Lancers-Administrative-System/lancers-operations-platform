...
| `PhoneField`                                                                  | Every phone number typed anywhere: country code, national number, and a confirm box beneath it. Only the first is stored; the second is nameless, reaches no action, and exists to refuse a submission whose two entries disagree. A number already on file that nobody touched is not retyped.     |
...
| `StepTrail`                                                                   | Where the reader is in a sequence they cannot see the end of. A step carrying an `href` is a link, the current one included; one without is plain text with its status word (LAN-362, replacing "a map, not a control").                                                                            |
