...
| `LAN-74` | `scripts/pilot/lan-74/` | 8 — 2 `people` (one deliberately first-name-only), 3 `contact_points`, 1 `season_memberships` in the **existing** open season, 2 `season_membership_status_events`. All ids in the block `00740074-0074-4074-8074-…`, all carrying `PILOT-LAN-74`. Creates **no** season | **No, not yet.** Proved against local Supabase only; Brian runs setup after the deployment is ready | Retain while returner intake is being exercised; cleanup when the scenario is no longer needed, and at the cutover |
