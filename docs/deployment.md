...
| `WHATSAPP_APP_SECRET`           | `whatsapp-app-secret`           | Verifies `X-Hub-Signature-256` on every inbound Meta callback (LAN-78), **and since LAN-360 signs every outbound Graph call**: the sender sends `appsecret_proof` — `HMAC-SHA256(key = this secret, message = the access token)` — and refuses to send at all without it, rather than sending unsigned. Without it `/api/webhooks/whatsapp` answers 503 to Meta's own delivery-status pushes and no message goes out. | **Provisioned**                   |
