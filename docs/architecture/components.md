...
| `band-colours.ts`           | `Band`, `BandColours`, `BAND_COLOURS`, `bandColoursFrom`, `bandColoursForSwatch`, `DEFAULT_ROSTER_GROUP_COLOURS`                    | Where a band's colour comes from: the ten roster groups from `roster_group_colours` (LAN-430)              |
| `band-colours-provider.tsx` | `BandColoursProvider`, `useBandColours`, `useRosterGroupColourKeys`                                                                 | The stored group colours, handed to every band by the `/operate` layout                                    |
...
| `field.tsx`                 | `Field`, `SelectField`, `ChoiceField`, `CheckField`, `DateField`, `TimeField`, `FieldProps`, `NO_AUTOFILL`, `preventImplicitSubmit` | Every form control, one size, full width; `NO_AUTOFILL` for a field about somebody else (LAN-324, LAN-332) |
| `multi-select-field.tsx`    | `MultiSelectField`, `GroupedMultiSelectField`                                                                                       | A control that takes several values                                                                        |
| `phone-field.tsx`           | `PhoneField`, `PhoneFieldProps`, `PHONE_CONFIRM_MISMATCH_MESSAGE`                                                                   | The one phone control (LAN-275): country, number, and a confirm box that refuses a mismatch (LAN-389)      |
...
| `step-trail.tsx`            | `StepTrail`, `TrailStep`                                                                                                            | Where the reader is in a sequence; each step links to itself when given an `href` (LAN-362)                |
