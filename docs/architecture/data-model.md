...
| `event_template_audience_groups`      | D47's default audience, stored as **groups** rather than people — a group is a way of selecting people, and the resolved list stays `event_audience_members`. D46 keeps `recruits` to the Recruitment type, as a check. `public.audience_group` gained `bps` (D-003, correction round 3, `WP-operator-record`, LAN-217): the audience already offered on the event's own picker is now pre-choosable on a template too, on every event type, the same way as every other group.                                                                                                                                                                                                                                                                                                                      |
