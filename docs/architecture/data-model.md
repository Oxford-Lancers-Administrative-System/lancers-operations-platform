...
| E1b | The confirmed audience is **non-empty**                                                    | **Service layer** — `resolveSelection` in `src/lib/services/audience-selection.ts`, refusing before the approval transaction opens                                                                                           | Yes, both sides: one test asserts the database accepts an empty audience, another that the service refuses one |
...
| --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A non-empty confirmed audience** (E1b)      | The database holds the audience and refuses an approval that records no confirmation, but "at least one row exists in another table" is not expressible as a constraint without a trigger, which the architecture record forbids for workflow. The approval transaction inserts the audience and flips the status together, and must refuse an empty one. Built in LAN-77: `resolveSelection` refuses before the transaction opens, and `approveEvent` in `src/lib/services/event-approval.ts` is the transaction. See [ADR 0012](../adr/0012-explicit-event-audience.md) for the declarative alternative considered and rejected. |
