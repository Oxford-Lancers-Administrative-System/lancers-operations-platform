...
| `delivery_attempts`                   | One outbound attempt and the provider's message identifier, recorded **before** the provider answers so an asynchronous callback can be matched to a job. `delivery_results` remains the authoritative outcome (M4). **LAN-394** adds three nullable columns — `safety_admitted_at`, `safety_person_id`, `safety_destination_key` — set together by a real admitted attempt and by nothing else, which is why the placeholder attempt LAN-252 writes for an unconfigured deployment counts for nothing. They are the whole of the safety accounting: the rolling windows are indexed counts over these rows, and there is no second ledger. `safety_destination_key` is a channel-namespaced SHA-256 fingerprint of the destination actually selected — sensitive and guessable, never anonymisation — kept server-side only and never in a log, an alert label or a browser payload. The two identifying fields are cleared together after eight days by the scheduler sweep, and immediately on erasure; `safety_admitted_at` stays, so the global accounting remains conservative for a row whose recipient is no longer recorded.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
