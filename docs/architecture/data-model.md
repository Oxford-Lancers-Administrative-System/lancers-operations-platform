...
| `person_fact_disputes`                | LAN-214, `REQ-no-silent-overwrite`. The disputed-fact raise-and-resolve pair Mission 5 deliberately left unbuilt (`REQ-no-disputed`). Scoped to the seven `people` columns `person-write.ts`'s `updatePersonField` overwrites in place; at most one **open** row per `(person_id, field)`, enforced by a partial unique index. The losing value stays on the same row, never deleted.                                                                                           |
...
- **`person_fact_disputes`** is the disputed-fact raise-and-resolve pair
  `REQ-no-silent-overwrite` needs and Mission 5 deliberately left unbuilt.
