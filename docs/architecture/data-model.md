...
| E1b | The confirmed audience is **non-empty**                                                    | **Service layer** — the database stores the audience but cannot require one without a trigger                                               | Yes: a test asserts the database accepts an empty audience, so the boundary cannot be misread |
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A non-empty confirmed audience** (E1b)      | The database holds the audience and refuses an approval that records no confirmation, but "at least one row exists in another table" is not expressible as a constraint without a trigger, which the architecture record forbids for workflow. The approval transaction inserts the audience and flips the status together, and must refuse an empty one. See [ADR 0012](../adr/0012-explicit-event-audience.md) for the declarative alternative considered and rejected. |
