...
no longer being asked. The key also carries `on update cascade`, which covers a
group the new class still permits and no further: a `recruits` row cascaded on
to a non-recruitment class fails
`event_audience_groups_recruits_are_recruitment_only`, and the type change is
refused after all. The service-side clear is what makes LAN-391's edit legal,
not a tidiness the cascade would have covered anyway.
...
- Every one of those references is **composite** — `(template_id, event_type)`
  ...
  what keeps
  `event_template_audience_groups_recruits_are_recruitment_only` and
  `messaging_schedules_recruit_fields_are_recruitment_only` meaningful: both read
  their own `event_type` column, and the composite key is what guarantees that
  column is the template's.
...
**Four rules the class still carries, and why it was kept.** D46's
recruits-on-Recruitment-only audience rule, the Monday report's buckets, coach
attendance, and the recruit messaging ladder all need a closed vocabulary, and
none of them can key off a name an operator may change. Every template carries
