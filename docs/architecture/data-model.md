...
Its other half — that the event's date has **passed** — cannot be a check
constraint, because a check constraint cannot read the clock. Since LAN-151
occurrence is derived rather than stored (D30: the date passed and the event was
not cancelled), so "has it happened?" is one service rule, `hasOccurred` in
`src/lib/services/event-input.ts`, and every reader asks it rather than
re-deriving it. `public.rsvp_attendance_mismatches` makes the same derivation in
SQL, against Europe/London.
...
| P5  | Attendance requires an event that has occurred                                                                                                                                             | Two halves. Database: composite FK + `event_status = 'approved'` check. **Service layer** for "its date has passed", which a check constraint cannot ask because it cannot read the clock — `hasOccurred`, used by every reader | Yes, both halves, including cancelling an event that carries attendance                                            |
