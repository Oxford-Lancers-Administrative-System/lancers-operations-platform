...
| `event_templates`                     | What an event created from it starts as. **Operators create, rename and delete these** (LAN-265, Brian 2026-09-09), so it is keyed by its own `id` and carries a case-insensitively unique `name`; `event_type` remains beside it as the behavioural class the code needs a closed vocabulary for, never shown to an operator and `practice` on anything newly created. Every other field is optional, so a field the template does not decide simply arrives empty on a new event (D41, D47). The seven rows the migration seeds carry fixed literal identifiers so scripts can name one without reading it back. Renaming is retroactive by decision; deleting is refused once any event names it. See [below](#event-templates-and-the-behavioural-class).                                        |
