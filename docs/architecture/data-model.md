...
Thirty-four tables, eight views and thirty-one enum types across twelve
migrations, plus three staging tables in an unexposed schema.
...
| Invitation           | `invitations`                                | surrogate        | → event (with status), season, membership **or** person | one per invitee per event                                | `invitation_status` enum                           | —                                               | Release one |
...
| `invitation_response_state`  | Invariant P7's partition; excludes non-soliciting events (E6)                |
| `nonresponse_queue`          | Requirement 6's escalation queue                                             |
...
| P7  | Five-way response state, reportable                        | `invitation_response_state` view                                                        | Yes                                                   |
| P8  | Player anchors to membership; others to person             | `*_anchor_matches_capacity` checks                                                      | Yes                                                   |
...
| E1  | Approval requires date, type and confirmed audience        | `events_approval_requires_date_and_audience` check                                      | Yes, both halves                                      |
...
| M1  | Unique idempotency key; atomic claim                       | `unique (idempotency_key)`; **service layer** for the atomic claim                      | Key tested                                            |
...
| M5  | Published report is an immutable snapshot                  | Insert-only table with no status column                                                 | Yes                                                   |
