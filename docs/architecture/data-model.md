...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ---------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------- | ----------- |
| Person                                                                                                                                                                                                                                                                                                                                                        | `people`                                     | surrogate `uuid` | self-FK `merged_into_person_id`                                              | none — no natural key (I1)                               | none; alumni standing derived by `person_standing`        | merge audited in place; `middle_name` LAN-366     | Release one |
...
| Recruitment Prospect                                                                                                                                                                                                                                                                                                                                          | `recruitment_prospects`                      | surrogate        | → `people`, `seasons`, conversion → membership                               | `(person_id, season_id)`                                 | `prospect_status` enum, seven values (LAN-201)            | `recruitment_prospect_status_events`, append-only | Release one |
| — Channel Presence (partial)                                                                                                                                                                                                                                                                                                                                  | `season_messaging_consents`                  | surrogate        | → `people`, `seasons`                                                        | `(person_id, season_id)`                                 | `messaging_consent_state` enum                            | current row; `audit_events` carries transitions   | Release one |
...
| — (LAN-387) `position_slot` gained `offence_backup` and `defence_backup`, so a player's backup position a side is a slot beside the primary and inherits S1/S3/S4 unchanged. The four special-teams slots are historical: the single Special teams position column and its values were dropped on 2026-09-16 (Brian) and LAN-374's squad table replaces them. |                                              |                  |                                                                              |                                                          |                                                           |                                                   |             |
...
| Onboarding Item                                                                                                                                                                                                                                                                                                                                               | `onboarding_items` + `onboarding_item_types` | surrogate        | → membership, type                                                           | `(membership, type)`; types `(season, code)`             | `onboarding_item_status` enum, `claimed` joined (LAN-214) | `onboarding_item_history`, append-only (LAN-214)  | Release one |
...
| Event Question                                                                                                                                                                                                                                                                                                                                                | `event_questions`                            | surrogate        | → `events`                                                                   | `(event_id, prompt)`                                     | —                                                         | `version` bumped on a real change (LAN-367)       | Release one |
...
| RSVP Response                                                                                                                                                                                                                                                                                                                                                 | `rsvp_responses`                             | surrogate        | → `invitations`                                                              | `(invitation_id, recorded_at)`                           | binary `rsvp_value`                                       | append-only; current via `current_rsvp`           | Release one |
| Question Response                                                                                                                                                                                                                                                                                                                                             | `question_responses`                         | surrogate        | → invitation, question (same event)                                          | `(invitation, question)` where live (LAN-367)            | —                                                         | current row; superseded rows retained             | Release one |
...
| Table                                 | Why it exists                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `position_vocabularies`, `positions`  | Frozen model §1.2 calls the position list "a versioned reference list". Making it data is what lets invariant S3 be a foreign key. A code is corrected **in place** rather than replaced: `position_assignments` holds the `positions` row's id, so changing `code` or `label` carries every assignment with it. LAN-401 does exactly that for `N/T` → `NT` and for `E`, which is labelled `Edge` rather than `End`, across every vocabulary present — one position must not be spelled two ways in one database. New codes are a different question and are added only to the vocabularies non-archived seasons use, because widening an archived taxonomy after the fact is what S3 forbids.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `onboarding_item_types`               | The model says item types are season-configurable. A per-season type table is how "configurable" is expressed without schema changes. Nothing in the application creates a season, so nothing in the application creates its types either: the eleven the club actually uses are written down once in `src/lib/services/onboarding-item-types.json`, read from there by the application, the local seed and the showcase plan, and seated by `scripts/production/baseline/season-2026-27.sql`. A season with none generates every membership in it with no onboarding items — the production defect of 2026-09-17 (LAN-396) — so the baseline refuses a run that leaves the season without them, the local seed refuses to hand over a dataset that reproduces it, and the roster board says so once rather than leaving a reader to find uneditable cells.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `coach_group_assignments`             | Which coaching groups a player trains with this season. LAN-387 made it a multi-select (Stewart's Coaching Assignments tab, Brian 2026-09-16): one row per group held, `unique (season_membership_id, coach_group)`, uncapped, and the vocabulary closed at last to Offense, Defense and Special Teams by `coach_group_assignments_group_in_vocabulary`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `kit_item_options`                    | LAN-375. What each issued-kit item allows — Clint's kit sheet, values and spellings verbatim (`Champro all porpose L` and `Schutt skill S` included: the sheet is the record). Player-Owned (LAN-421) is the one value not on that sheet: it sits last on each of the five items Kit Distributed reads, and on no other. Reference data, read and never written by the application.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `kit_issue_records`                   | LAN-375. Which kit a player was issued this season — one row per filled item, value proved against that item's own list by composite foreign key. Braces L and Braces R (LAN-409, Brian 2026-09-21, renamed from `braces_1`/`braces_2`) each hold a _set_ of the same twelve brace values, so they take several rows for one item; `kit_issue_records_one_per_single_item` keeps every other item to one row, and `kit_issue_records_one_per_item_value` keeps a side's set a set. There is no count field anywhere. Not an inventory. A trigger on this table maintains the derived Kit Distributed onboarding item — see the invariant note below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `person_erasure_signoffs`             | LAN-361. The two confirmations an anonymisation needs, while they wait for their pair. One signer, one sign-off; deleted once the erasure completes, because a list of who asked to disappear is itself a list of who asked to disappear.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `special_teams_squad_positions`       | LAN-374. Which positions each special-teams squad allows — Stewart's Special Teams Assignments tab, values and order verbatim. Reference data, read and never written by the application: changing what a squad allows is a migration, exactly as a position vocabulary version is.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `special_teams_assignments`           | LAN-374. One player's pick for one `(squad, slot)` cell — six squads, four slots each, twenty-four cells. The value is proved against the squad's own list by a composite foreign key. Blank is the absence of a row. No cross-cell rule: this is a record of what a player can be asked to do, not a depth chart, so the same position may stand in several slots and several squads.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `membership_position_groups`          | LAN-387. Which position groups a player is assigned to, per side — Offensive Line / Quarterbacks / Runningbacks / Wide Receivers, and Defensive Line / Linebackers / Defensive Backs. Several per side, uncapped, keyed `(season_membership_id, side, position_group)`; `side` is `position_side` and is never `special_teams`, because special teams is squads and slots, not groups.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `warmup_small_groups`                 | LAN-401. The eight warmup small groups — Stewart's list, via Brian 2026-09-21: Kings, Raider, Bear, Phoenix, Cavalier, Blue, Gold, Lancer, names and order verbatim. Reference data, read and never written by the application: a ninth group is a migration, exactly as a kit size or a special-teams squad list is.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `warmup_group_assignments`            | LAN-401. Which warmup small group a player is in this season — at most one, `unique (season_membership_id)`, the value proved against `warmup_small_groups` by foreign key rather than by a check constraint. Blank is the absence of a row. Nothing is derived from it and no rule ties it to a position, a coaching group or a special-teams squad: it is where a player warms up, and says nothing about what they play.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `formalwear_records`                  | Tie, bowtie and socks, reasked every season (2026-08-26) rather than carried. `ownership` is text and not a boolean because "Yes (paid)" is a real answer the club records and prices the subscription invoice against. LAN-387 moved formalwear into the roster's Kit group and narrowed what the application reads and writes to `tie` and `bowtie`; the `socks` enum value and any row already recorded under it stay, unread.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |     |
...
| `event_audience_members`              | The frozen model gives Event an _audience definition_, and invariant P7 needs `never-invited` to be reportable. A repeating attribute is a table; without it the database cannot name anyone the approver confirmed. **One human, one row** since LAN-294 — see [below](#one-human-one-row-and-one-invitation). `added_by_group` (LAN-392) names the stored group rule that added the row after approval, and is null where the approver confirmed it or an operator added it by hand; with `added_by_person_id` also null it is the pair that says _this rule, no operator_, and it is what the retraction inside the grace window is allowed to remove and the five-an-hour cap counts. **LAN-414** moves that predicate to `added_by_group_category`, which is set for every rule add whatever category the group belongs to; `added_by_group` still names a General group and `added_by_group_value` names a sub-group's roster value, and `event_audience_members_added_by_group_representation` holds exactly one of the two true.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `operator_preferences`                | LAN-387, Brian's visual pass of 2026-09-17. One operator's own screen settings, keyed to the Person their session resolves to. Not a club concept and read by nothing in the domain — no audience, message, report or export reaches it. One row per person (`unique (person_id)`) carrying one `jsonb` object, rather than a column per setting: a preference has no invariant to enforce, and the whole object is read and written together by one screen. The one rule enforced is that it is an object (`operator_preferences_is_object`). Today it carries `rosterCollapsedGroups`, the roster board and membership record's folded-up column groups; an absent key means the screen's own default. Untouched by erasure (LAN-361 anonymises and never deletes, so its `on delete cascade` never fires): the row carries no fact about the human at all, only which columns their browser had folded away.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `role_access_grants`                  | LAN-429 (LAN-423, Brian 2026-09-25). What each seat may see and change, one row per (seat, line), edited on the seat page by `role_management` holders. `subject_kind` is `roster_category` (twelve keys: the board's ten groups plus `contact_emergency` and, since `20261007090000_attendance_access_line.sql`, `attendance`), `recruiting_category` (`recruit_person`, `recruit_details`, `recruit_events`), `event_template` (by `template_id`, **`on delete cascade`**, no key) or `switch` (`add_to_roster`, `add_recruits`); `level` is `none`/`view`/`edit`, `none`/`view` for `attendance` and `recruit_events`, `none`/`view`/`manage` for a template, `none`/`yes` for a switch — all pinned by `role_access_grants_vocabulary` and `role_access_grants_subject_shape`. One line per (seat, subject) by a `nulls not distinct` unique constraint. Seeded with every seat on every line: President, General Manager, IT Officer, Vice-President and Secretary at the maximum, every other seat at `none`; a template created later gets `manage` for the three fixed seats and `none` for every other in its creating transaction. An operator's access is the union-maximum over their current seats, read once per request. The floor (President, General Manager, IT Officer fixed) is a service rule, not a row. `service_role` holds `select, insert, update`; no delete — a line leaves only with its template. See [below](#seat-access-grants-lan-429).                                                                                                                                                                                                                                                                                                                                                     |
| `roster_group_colours`                | LAN-429 (W2 of LAN-423). The colour each of the roster board's ten groups wears, as a palette key from `TEMPLATE_COLOUR_PALETTE` — never a hex value. `group_key` is the primary key over the ten groups (Contact & emergency has no colour of its own; its record section wears Person's). Seeded from the packet: person and membership `blue` (Oxford Blue), onboarding and kit `lancer_gold`, availability `slate`, coaching `indigo`, offensive `teal`, defensive `purple`, special teams `brown`, warmup `cyan`. Edited by `role_management` holders; each save is one `roster.group_colours.changed` audit row. `service_role` holds `select, update`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `messaging_safety_scopes`             | LAN-394, Brian 2026-09-17. Durable safety state for the club's outbound messaging: one `global` row, one per `provider` (`whatsapp`, `email`), and one per `person` or `destination` that has been held — a recipient row is written only when a hold is actually recorded, an ordinary admission reads an absent row as "no hold" and creates nothing, and the scheduler sweep deletes a recipient row again once it has held nothing for eight days. Machinery, not a club concept — it records switches and circuit state, never a fact about a human, and nothing in the domain reads it. `paused_at` (a person deciding) and `latched_at` (a threshold tripping) are separate columns so resuming one never clears the other; `version` is optimistic concurrency, so a stale browser cannot undo a newer incident. `scope_key` is deliberately text and deliberately not a foreign key, even for a person: a merge combines these explicitly (`person-merge/write.ts`) and an erasure removes them (`person-erasure/anonymise.ts`), by the person id and by the destination fingerprints of the person's own contact points, computed before those contact points are deleted so that a hold outlives neither the eight-day clearing of `delivery_attempts.safety_destination_key` nor the erasure itself. Circuit state (`consecutive_faults`, `cooldown_until`, `cooldown_stage`, `probe_generation`) is constrained to `provider` scopes, and the alert bookkeeping (`incident_alert_at`, and `capacity_alert_at`/`queue_alert_at` on the global row only) is what keeps the count-only monitoring route from repeating an open incident more than once an hour. The thresholds themselves are **not** here: they are named constants in `src/lib/services/messaging-safety/policy.ts`, shown read-only on the page. |
...
| `event_type_settings`                 | Per-template configuration LAN-151 stores and **Mission 4** consumes: the chase thresholds D75 and D77 name. **Keyed by `template_id` since LAN-265** — one row per template, created with it and deleted with it by the cascade; `event_type` stays beside the key as the behavioural class, held equal to the template's own by a composite foreign key. Not on the template row itself: W8 is explicit that a template says what an event arrives looking like, and a chase threshold is not part of what an event is.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `event_templates`                     | What an event created from it starts as. **Operators create, rename and delete these** (LAN-265, Brian 2026-09-09), so it is keyed by its own `id` and carries a case-insensitively unique `name`; `event_type` remains beside it as the behavioural class the code needs a closed vocabulary for, never shown to an operator and `practice` on anything newly created. **`colour_key`** (LAN-276 correction round 1, Brian 2026-09-10) is a `not null` palette key — never a hex value — chosen on the editor and checked against the fixed list `event_templates_colour_key_known` restates from `src/lib/services/event-template-input.ts`; the calendar legend, every calendar tile and every event list colour by it, so a template no longer shares Practice's swatch by accident. Every other field is optional, so a field the template does not decide simply arrives empty on a new event (D41, D47). The seven rows the migration seeds carry fixed literal identifiers so scripts can name one without reading it back. Renaming is retroactive by decision; deleting is refused once any event names it. See [below](#event-templates-and-the-behavioural-class).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `event_template_questions`            | D42's default questions, keyed to the template by `template_id` since LAN-265. Copied onto a new event and marked `from_template`, so removing one per event never touches the template.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `event_template_audience_groups`      | D47's default audience, keyed to the template by `template_id` since LAN-265 and stored as **groups** rather than people — a group is a way of selecting people, and the resolved list stays `event_audience_members`. **LAN-414** adds `category` (`public.audience_group_category`) and `value text`, and makes `audience_group` nullable: a `general` row carries the enum key, every other category carries the roster value verbatim, and `event_template_audience_groups_representation` refuses anything that is neither. The table gains a surrogate `id` primary key for the same reason `event_audience_groups` has one — a primary key's columns cannot be nullable — and its natural key becomes `unique nulls not distinct (template_id, category, audience_group, value)`. D46's `event_template_audience_groups_recruits_are_recruitment_only` is **dropped** by LAN-416: a recruit pill is offered on every template. `public.audience_group` gained `bps` (D-003, correction round 3, `WP-operator-record`, LAN-217): the audience already offered on the event's own picker is now pre-choosable on a template too, on every event type, the same way as every other group. It gained `onboarding` the same way (LAN-388, Clint and Brian 2026-09-17): somebody whose current-season membership is mid-onboarding is their own group rather than part of Active, offered on every event class an Active player is offered on, invited on the player ladder, and pre-choosable on a template like any other group.                                                                                                                                                                                                                                                                                           |
| `event_audience_groups`               | LAN-392. The derived audience groups **one event** was built from, kept alive past approval: a person who joins one of those groups later is added to the audience and invited under the event's own plan. The per-event analogue of `event_template_audience_groups`, keyed the same way on `(event_id, audience_group)` — as a unique constraint beside a surrogate `id`, because the owner-run showcase rollback walks foreign keys to `public.people` and deletes what it finds by `id` — carrying the same `event_type` copy under a composite foreign key (`on update cascade`, which covers a group the new class permits; a `recruits` row is refused, so `updateEventDraft` prunes first) so the class is provably the event's own. **LAN-414** gives this table the same `category`/`value` pair, the same `unique nulls not distinct (event_id, category, audience_group, value)` key and the same `event_audience_groups_representation` check; **LAN-416** drops `event_audience_groups_recruits_are_recruitment_only`, which was the one thing standing between an operator and an Engaged-recruits audience on a Training event. Written by `saveEventAudience` beside the audience, because a pressed group cannot be recovered from the people it resolved to — `summariseAudienceGroups` reports a group only when every person it would invite is selected, so unticking one name makes the group vanish from every derivation of it. Inert until the event is approved, and inert again once it has started.                                                                                                                                                                                                                                                                                              |
| `event_audience_exclusions`           | LAN-392. The people who were in one of an event's chosen groups when its audience was saved and were **not** selected — a deliberate deselection (Brian, 2026-09-17: "added as a group and you naturally fall into that group, you get it; deselected specifically, you do not"). Keyed `(event_id, person_id)` — again a unique constraint beside a surrogate `id`, for the same rollback reason — and not by group: no operator unticking a name is thinking about which group the name came from. The group rule never adds an excluded person to that event, however their standing changes afterwards; an operator adding them by hand (LAN-393) is the one door that clears it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `recruitment_signup_codes`            | LAN-201, REQ-qr-per-season. One live sign-up code per season, mintable, deactivatable and re-mintable. Stores the plaintext code rather than a digest, deliberately: unlike a personal invite link this is a shared, printable code the operator's page has to redisplay indefinitely, and knowing it opens only the public sign-up form. `sign_in_count` counts completed sign-ups on the code; LAN-428 added `visit_count`, the times `/join/[code]` was served for it while live, incremented server-side in the statement that resolves the code (`recordRecruitmentSignupVisitIn`). Both are counters on the code's own row, under the table's existing RLS and its `service_role` `select, insert, update` grant.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `season_messaging_consents`           | LAN-201, packet amendment 1. The season-scoped consent gate every send checks, keyed `(person_id, season_id)`. This is Channel Presence's early, partial landing out of the frozen model's §1.2 deferral — see [below](#release-one-versus-structurally-present) — authorised by Brian's 2026-08-31 owner amendment, not a quiet migration. LAN-371 added `reason`: why an operator changed it, required of every operator-recorded change and null for a change the person made themselves. `recorded_by_person_id` set is what tells "Revoked (by operator)" from "Revoked (by the person)".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `onboarding_item_history`             | LAN-214, `REQ-item-history`. `onboarding_items` carries current state only; this is the typed home for every transition — actor, date, the state pair — that table has no room for. Append-only: `service_role` holds `select, insert` and nothing else.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `onboarding_activity_log`             | LAN-214, `REQ-activity-log`. One row per ask, one per answer, grouped by `section` — never a count (`OD7-log-by-section`). Append-only, the same posture as `onboarding_item_history`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `onboarding_agreement_versions`       | LAN-214. The versioned slot the Code of Conduct and the photo release's real wording (Clint's, LAN-213) drops into. Seeded with a labelled placeholder per document type; never invented club policy. Insert-only — a version is added, never edited.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `onboarding_agreements`               | LAN-214. Version, moment and person — one row per `(person, season, agreement_type)`, seasonal by the item-and-ask inventory's own item 11. No object storage; the application has none.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `onboarding_chase_settings`           | LAN-214, `W11`. A singleton (`id boolean`, `check (id)`) holding the chase's three configured values — first-chase delay in hours, count, interval in days. No "give up after" column by design (`OD7-cadence-is-the-config`) and no escalation-office column: that office is read from `roles`/`role_assignments` through `messaging-scheduler.ts`'s existing resolver, never configured here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `bps_selections`                      | LAN-214. Item 5 of the item-and-ask inventory — a roster attribute, not a checklist item (Brian, 2026-09-01). One row per membership per season, on the `blues_awards`/`formalwear_records` seasonal-attribute idiom.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `person_fact_disputes`                | LAN-214. Built for the disputed-fact raise-and-resolve pair `REQ-no-silent-overwrite` named (Mission 5 deliberately left it unbuilt, `REQ-no-disputed`), then retired before shipping: `Q-9` (Brian, 2026-09-04) withdrew the whole mechanism — last-write-wins is the rule now, carried by the ordinary audit trail. **Present in the schema, written by nothing.** No migration dropped it (a live table with no writer is not a schema defect), and `person-merge.ts` still closes its own merge-time re-point gap for any historical row a merge might encounter. Scoped to the seven `people` columns `person-write.ts`'s `updatePersonField` overwrites in place; at most one **open** row per `(person_id, field)`, enforced by a partial unique index that nothing inserts against any more.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **The application may read the catalogue and may not write it.** `service_role`
  ...
  which is how "administrators cannot create roles in the application" is held.
  Changing a seat is a migration, reviewed on its own. What a seat may _do_ is a
  different table since LAN-429 — `role_access_grants`, below — and that one the
  application does write.

#### Seat access grants (LAN-429)

`20261006090000_granular_access.sql` (mission M-GRANULAR-ROLES-AND-PERMISSIONS,
parent LAN-423) moves the part of access the committee changes out of the
capability map and into `public.role_access_grants`. Four decisions carry it:

- **One row per (seat, line), every seat on every line.** The seed writes all
  of them, so "no row" is never the normal state of a line; a missing row still
  reads as `none` (absence of a decision is never permission). Template lines
  cascade with their template, and `deleteEventTemplate` records the lost lines
  in its own audit row.
- **Vocabulary by check constraint**, restating `src/lib/auth/grants.ts`, the way
  `event_templates_colour_key_known` restates the palette. A new category is a
  forward migration and Brian's decision.
- **The floor is a service rule.** President, General Manager and IT Officer
  hold every line at its maximum; `src/lib/services/access-grants.ts` refuses
  any write to them (`access_fixed_seat`). No trigger enforces it, because the
  refusal must be a `NotPermitted` the seat page can report.
- **Audit in `audit_events`, family `access`.** `administration.access.changed`
  (one line, levels as from/to state), `administration.access.copied` and
  `administration.access.granted_all` (one row, one line per change in
  `detail.changes`), filed against the seat (`entity_table = 'public.roles'`),
  so `readHolderHistory` shows them in the seat's History. The printed-access
  test (`tests/printed-access.test.ts`) pins the seeded matrix to a checked-in
  fixture.

`roster_group_colours` rides in the same migration: ten rows, one per board
group, each a palette key.

`20261007090000_attendance_access_line.sql` (LAN-423 round 6, Brian 2026-09-26)
adds the twelfth roster category, `attendance`, at `none`/`view` only: it
governs the player record's Attendance section and nothing else (recording
...
- **`messaging_schedules`** — one row per template since LAN-265, holding
  ...
  Complete with **no default arm**, and the rekey strengthened rather than
  weakened that: it used to be "complete over the enum, and an event type with no
  row is a refusal that names itself", enforced by a seed and a service check.
  `messaging_schedules_pkey` on `template_id` and the cascade on
  `messaging_schedules_template_fkey` now make a template with no cadence
  unrepresentable, and `createEventTemplate` writes both rows in one
  transaction. `event_type` stays on the row as the behavioural class, which is
  what `messaging_schedules_recruit_fields_are_recruitment_only` still reads.
  It carries no
...
LAN-394 adds three more, and each one exists precisely because none of the four
above means what it means. `safety_retry_at` is the earliest moment the safety
guard is worth asking again — never a delivery retry, because no attempt was
made and none was consumed. `safety_block_scope_id` names the scope currently
holding the job back, so the sweep can skip it before applying its limit and the
page can say what the hold is on. `safety_reason_code` is a code from a fixed
list, never a provider sentence: `last_error` means "what the provider said
about a delivery that was attempted", and a message that is waiting was never
attempted. Writing a safety deferral into `last_error`, `next_attempt_at` or
`held_at` would make a queued message read as a failure, a late rung, or an
amendment hold, which are three different things an operator would act on
differently. Still no seventh status: a waiting job is `pending`.
...
- **The status ladder gained a rung, and lost two names.** `prospect_status`
  ...
  `converted` renamed to `joined`, `lapsed` to `disengaged`, and `void` is the
  seventh value — a marker that the record itself is wrong, never a statement
  about the person (Addendum A to LAN-201). The Lead's decision binds over
  `DEC-void-is-a-marker`'s open question and `W13`'s recommendation of a
  separate marker column: `REQ-status-ladder` is explicit that there is
  exactly one `prospect_status` field, so `void` is a value, not a second
  column. Whether a `void` record still shows on the board is a display rule
  read off this one field, decided by a later package — the schema enforces
  only that a `void` transition is always explained (see
  `recruitment_prospect_status_events` below). The two constraints on
  `recruitment_prospects` — `..._conversion_matches_status` and
  `..._commitment_is_dated` — are unchanged in meaning, re-added against the
  renamed values.
- **Notes gained an author and a date.** `recruitment_prospects.notes` was one
  prose column; `recruitment_prospect_notes` is now a repeating attribute table
  on the `season_membership_status_events` actor idiom (a person or a named
  process, never blank). The prior column's content was migrated into it
  rather than dropped, and the column is gone.
...
- **Questionnaire answers** — `recruitment_questionnaire_responses` — carry
  ...
  questionnaires are still an open, `proposed for owner approval` decision in
  `W4`. Attributed to the recruit, never to the club (Addendum A); a later
  answer supersedes rather than overwrites, so the earlier one is kept.
- **The season sign-up QR** — `recruitment_signup_codes` — one live code per
  season, mintable, deactivatable and re-mintable. It stores the plaintext
  code rather than a digest: unlike `person_access_tokens` or
  `rsvp_access_tokens`, this is a shared, printable code an operator's own page
  has to redisplay indefinitely, and reaching it opens only the public sign-up
  form. Two counters ride on the code's row: `sign_in_count` (completed
  sign-ups) and, since LAN-428, `visit_count` (page loads of `/join/[code]`
  while the code is live).
- **Season-scoped messaging consent, and Channel Presence's early landing** —
  `season_messaging_consents`, keyed `(person_id, season_id)`, is the one gate
  every send checks regardless of channel — which is what makes "an opt-out
  honoured across channels" (Meta's requirement) true of a single table rather
  than one per channel. Frozen model §1.2 defers Channel Presence from the
  release-one schema; this table is that entity's first, partial landing,
  authorised by Brian's 2026-08-31 owner amendment (packet amendment 1) rather
  than a quiet migration. **Known limitation:** `person-merge.ts`'s merge does
  not yet combine two identities' consent rows for the same season the way it
  does for `recruitment_prospects` — `season_messaging_consents.person_id` is
  declared but excluded from the plain re-point set for exactly that reason,
  tracked for the package that builds consent behaviour on this table.
- **The recruit ladder joined the messaging schedule's own row.** Per
  `DEC-split-on-the-schedule`, `messaging_schedules.recruit_invitation_lead_days`
  and `recruit_follow_up_cadence_hours` are two new columns, not a second row:
  null for every event type but `recruitment`, where they hold the Recruits
  audience's own "first invitation, one follow-up, no escalation field at all"
...
- **The recruit ladder joined the frozen plan.** `event_messaging_plans`
  gained five `recruit_*` columns — `recruit_invitation_lead_days`,
  ...
  event. All five are nullable together — most approved events carry no
...
### Erasure is anonymisation, never deletion (LAN-361)

A person who asks to be erased is **tombstoned**. `people.erased_at` is stamped,
and `people_erased_rows_carry_no_identity` — a check constraint, not a promise —
refuses a stamped row that still carries a family name, a middle name, a
college, a degree field, a matriculation or graduation year, a date of birth, a
student number, a BAFA number or a merge reason. `given_name` becomes a fixed
label so every screen that still lists them has something to print.

Nothing is deleted from `people`. Every record of what the club did — attendance,
RSVPs, agreements, delivery results, status events, audit rows — keeps its
foreign key and points at the tombstone. Deleting the row would take a season's
history with it, and a person asking not to be identifiable is not asking for
that.

Three tables are deleted outright, because they are the person's contact details
and nothing else: `contact_points`, `person_aliases` and
`person_emergency_contacts`. Every live `person_access_tokens` and
`rsvp_access_tokens` row is revoked, every queued `notification_jobs` row is
cancelled, and every free-text and JSON column that could name them is replaced
with a fixed marker. `src/lib/services/person-erasure/anonymise.ts` names each
table explicitly; `src/lib/services/person-erasure.test.ts` reads the catalogue
and scans **every** text and JSON column of **every** table in `public` and
`staging` for the seeded person's own values, so a table added later that keeps
a name fails a test.

`person_erasure_signoffs` holds the confirmations while they wait for their
pair, keyed `unique (person_id, signed_by_person_id)` — which is the whole of
"the second confirmation cannot come from the first signer". The rows are
deleted once the erasure completes; what happened is in the one audit event,
which names person ids and seats and no personal data at all.
...
group's red flag, Yes or No — but no operator sets it. It reads `complete` when
all five of Helmet, Shoulder Pads, Lower Pads, Lowers and Practice Jersey carry
a `kit_issue_records` value, and `pending` otherwise. Team Mouthguard is
deliberately outside the rule (Brian, 2026-09-16).

**`Player-Owned` is one of those values** (LAN-421, Stewart's ask and Brian's
decision of 2026-09-22; migration `20261005090000_kit_player_owned.sql`). It is
a `kit_item_options` row on each of those five items and on no other, sorted
after that item's existing values, so a player who owns their own helmet can
complete the flag without being recorded against a size the club never handed
them. The derivation is unchanged, and deliberately so: the rule counts any
value, so reference data was all this needed. The migration is data only — no
table, column, type, constraint, policy or grant moves, and
`database.types.ts` is unchanged by it.
...
It is a real row rather than a view because the board's onboarding summary, the
outstanding-items notice and the chase all read `onboarding_items` already. The
...
| P7  | Five-way response state, reportable (six since LAN-392's `never_asked`)                                                                                                                    | `event_audience_members` supplies the population; `invitation_response_state` left-joins invitations to it, so `never-invited` is derivable, and `message_withheld_reason` separates an invitation nobody was sent from one nobody answered             | Yes — all five states, `never_asked`, plus the negative case that a non-audience member is not reported              |
...
| P9  | **One human, one audience row and one invitation per event** (LAN-294, Brian 2026-09-10)                                                                                                   | `event_audience_members.invitee_person_id` — the human, denormalised and kept true by a composite FK — under a total `unique (event_id, invitee_person_id)`. Invitations inherit it. See [below](#one-human-one-row-and-one-invitation)                 | Yes — both anchors, the negative case, both ways the copy could lie, and one invitation for a person in three groups |
...
| **A superseded question answer**      | `question_responses.superseded_at`, `superseded_reason` and `question_version` (LAN-367). A question whose wording, answer type or choice options change after the invitation went out voids the answers it collected: the row keeps what the person said — the record of what was answered to the older version — and every current read filters on `superseded_at is null`, so the question reads as outstanding again. Uniqueness moved to a partial index over the live rows, so re-answering does not collide with the history. Removal is still refused on an approved event (LAN-318), so a superseded answer today is always a changed question rather than a removed one.                                                                                                                                                                                                                                                          |
| **Middle name**                       | `people.middle_name` (LAN-366). Nullable, with the same not-blank check the other three name columns carry. Entered on the person record, the operator's create-person form, the onboarding details step and an optional `middle_name` column on the roster import; the QR sign-up door stays given and family name only. Shown on the person record and its edit form and nowhere else — every list, board, template, greeting and the BAFRA form keeps the display name it already uses — and it is never part of duplicate matching.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
**LAN-265 reopened what a template is**, and the rest of this section is what
that costs the map.

## One human, one row, and one invitation

Invariant P9, added by LAN-294. Brian, 2026-09-10, walking the G7 review
environment: "One invitation per person per event." A person who plays, coaches
and sits on the committee is one person.

**What was wrong.** `event_audience_members` already carried two unique indexes
that read as though they said this:
...
Both are partial, and invariant P8's `anchor_matches_capacity` forces a player
row to fill only the first column and every other capacity only the second. One
human admitted as a player **and** as a committee member therefore violates
neither: between them the pair says "one row per invitee _per anchor_". Two
seeds wrote exactly that pair for anybody who plays and also holds a seat, and
on the seeded club 840 person/event combinations carried two audience rows and
two invitations each — visible to Brian as Bertram and Caspian counted twice in
the confirmed audience and listed twice in the participation table.

**The device, and why it is not a trigger.** The fact that has to be unique is
_the human_, and for a player row the human is only reachable through
`season_memberships`. A check constraint cannot read another table, so the
obvious reach is a constraint trigger — which ADR 0008 rejected by name and
ADR 0012 rejected again for this table. ADR 0008 also names what to do instead,
and this is that device exactly: the child carries a copy of the parent's
discriminator, joined back by a composite foreign key so the copy cannot lie.
`season_memberships` already published `unique (id, person_id)` for it.

- `event_audience_members.invitee_person_id` is `not null` — the human the row
  is about. It is **not** a second anchor: P8 is untouched, `person_id` is still
  null on a player row, and `participant_id` still generates from the anchor.
- `event_audience_members_invitee_holds_the_membership` — the composite foreign
  key to `season_memberships (id, person_id)`. `MATCH SIMPLE` skips it for a
  non-player row, whose `season_membership_id` is null.
- `event_audience_members_invitee_is_the_anchor_person` — the check that covers
  those rows: `person_id is null or invitee_person_id = person_id`.
- `event_audience_members_one_per_human_per_event` — a plain, total
  `unique (event_id, invitee_person_id)`. This is the rule.

**Invitations need nothing of their own**, which is worth stating rather than
leaving to be re-derived. Every invitation names the audience member it was
resolved from, and `invitations_belong_to_the_resolved_audience` binds it to that
row's event, capacity and participant — so an invitation cannot describe a human
the audience does not already hold exactly one row for. One row per human per
event therefore _is_ one invitation per human per event.

**The service layer collapses first**, in `resolveSelection`
(`src/lib/services/audience-selection.ts`), by `CAPACITY_PRECEDENCE` — player
before coach before committee before recruit — so the capacity written is the
highest the person holds. The index is the backstop for every other writer: a
seed, a pilot script, a correction by hand, a later issue adding a late invitee.

**Applying it to a populated database.** The unique index is built last and will
refuse to build while any event still holds two rows for one human. That is
deliberate — silently keeping them would leave the club messaging one person
twice with nothing on any screen to say so. Both seeds were corrected in the same
change, so `db:reset` + `db:seed` produces none; a database seeded before it must
be reset, and a hosted environment must be checked for the pair before the
migration is applied.

## An approved event keeps its audience groups (LAN-392)
...
**What the audience freeze now means.** ADR 0022 recorded that both audience
write paths guard on `status = 'draft'`, so "the audience is frozen the instant
the event is approved — structurally, not by omitting a button". That sentence
is amended, not abandoned: it now means _the approver's list is never reduced
and never re-resolved_, rather than _the audience never grows_. Nobody is ever
removed by a rule, no confirmed row is ever rewritten, no capacity is ever
upgraded, and the rule cannot reach a person the approver unticked. ADR 0022
carries a dated amendment note saying exactly this.

**The two new tables, and why neither is derivable.** `event_audience_groups`
holds which group buttons were pressed and `event_audience_exclusions` holds who
was taken out of them. Neither can be recovered afterwards from
`event_audience_members`: `summariseAudienceGroups` reports a group only when
_every_ person it would invite is selected, so unticking one recruit removes the
group from the summary entirely, and "deselected on purpose" and "not in the
group at the time" become indistinguishable in both directions.

**What a late joiner receives.** The invitation at their own moment, then the
event's ladder exactly as it already stands. `event_messaging_plans` stays one
row per event — its own migration says "Not per invitation: every invitee of one
event is on the same ladder" — so nothing per-person is stored. The late
joiner's reminder jobs are inserted at the event-wide rungs still in the future,
carrying the same `ladder_rung`, which is what makes an amendment's reschedule
move them with everybody else's and a cancellation stand them down with
everybody else's, neither knowing they exist. Their `expires_at` is the event's
own `response_deadline_at`, unchanged: a per-person deadline would leave two
people on one event holding two expiries while the event page showed a third.

**Timing, and the two guards.** No auto-add message leaves inside ten minutes of
the status write, so a flip put back inside the window takes the audience row,
the invitation and the job away again with it — cancelling the job alone would
leave a pending invitation in the non-response queue being chased about a
message nobody sent. At most five auto-add invitations reach one person in an
hour, measured from their own earliest queued auto-add send; the rest queue an
hour apart. Both are expressed by choosing `scheduled_for`, because there is no
send-time throttle anywhere in this system. An unconsented recruit gets the
audience row and the invitation and no declared job, checked where
`scheduleEventLadderIn` checks the same thing.

**An invitation nobody was ever sent.** The two cases above — no consent, and an
event that starts before the invitation's own send time — leave a `pending`
invitation with no job behind it, and a pending invitation reads as
`awaiting_response`. That would put somebody nobody messaged into
...
referenced value: while a group row names the old class, the `update` that
changes a draft's type is refused outright. `updateEventDraft` therefore clears
this draft's groups and exclusions before it writes the new class — a class
change re-applies the new template's defaults, so both are answers to a question
no longer being asked. The key also carries `on update cascade`. Until LAN-416 that cascade
covered a group the new class still permitted and no further — a `recruits` row
cascaded on to a non-recruitment class failed
`event_audience_groups_recruits_are_recruitment_only`, and the type change was
refused after all. That check is gone and no group is now type-scoped, so the
cascade would succeed on its own; the service-side clear stays, because a class
change re-applies the new template's defaults and the old rule is an answer to a
question no longer being asked.

**The chokepoint.** Every service write that can move a person between derived
audience groups calls one function, inside its own transaction, and
`tests/audience-group-rule-writers.test.ts` enumerates them from the source and
...
(Brian's decision 10). A template default is not the approver's answer: where an
approver pressed a group and then unticked somebody, that untick is not
recoverable — no exclusion exists to backfill — so on those older events the rule
may later add a person who was deliberately left out. It cannot happen to an
event approved after the migration, because from then on the exclusions are
recorded at the same moment as the groups. Every approved event is backfilled
whatever its date, rather than only future ones, so that the migration's result
does not depend on the minute it is applied; the rule's own predicate keeps a
past event inert.

## Event templates and the behavioural class

Brian, with Stu and Clint, 2026-09-09: "A template is anything the operators want
to create: 'Kicking Clinic', 'Full Pads Practice', 'Film Review', whatever they
name. It is not a preset of a fixed type." That is a decision about the domain,
and it moves one thing in this map: `public.event_type` stops being the identity
of a template and becomes its **behavioural class**.

`20260916090000_event_templates.sql` is the whole of the change.

- `event_templates` gains `id` and `name`; the primary key moves to `id`, and
  `name` is unique case-insensitively because it is the only thing an operator
  ever sees of a template.
- `messaging_schedules`, `event_type_settings` and `events` gain `template_id`,
  backfilled by class, and the two primary keys and the two template foreign
  keys move onto it.
- `event_type` survives as a plain column on all of them, so a row can still
  answer "what class of event is this?" without a join.
- Every one of those references is **composite** — `(template_id, event_type)`
  against `event_templates (id, event_type)` — so the denormalised class is
  provably equal to the template's own rather than conventionally equal. That is
  what keeps
  `messaging_schedules_recruit_fields_are_recruitment_only` meaningful: it reads
  its own `event_type` column, and the composite key is what guarantees that
  column is the template's. Its sibling
  `event_template_audience_groups_recruits_are_recruitment_only` was the other
  such check until LAN-416 dropped it.
- `events.template_id` references with **`on delete restrict`**, which is what
  "delete when unused" means at the schema level: an event whose template
  vanished could not say what kind of event it is, because after LAN-265 the
  template's name is the only label there is.

**Three rules the class still carries, and why it was kept.** The Monday
report's buckets, coach attendance, and the recruit messaging ladder all need a
closed vocabulary, and none of them can key off a name an operator may change.
D46's recruits-on-Recruitment-only audience rule was a fourth until LAN-416
retired it; what survives of it is the _cadence_, which is still keyed on this
class and on nothing else. Every template carries
one; the seven the migration seeds keep theirs; anything an operator creates gets
`practice`. It is never shown on any screen and never chosen by an operator —
adding a class remains a migration and Brian's decision.

D46 in particular is now read at the level that matters, which it was not before
LAN-295. It used to live only in `AUDIENCE_GROUPS`, where it withheld the
_Recruits button_ while the recruits themselves stayed in the catalogue on every
event type as individually tickable rows — so an operator could put six prospects
on a Wednesday practice and approval would invite them. `listAudienceCatalogueIn`
now takes the event's own `events.event_type` and returns the recruit arm only
for `recruitment`, which is the one read the picker, the template default, the
approval preview and the approval write all share. Brian, 2026-09-10: "Recruits
should only ever be selectable and only ever be available for a recruitment
event. Every other event, they're non-factors."

**Renaming is retroactive, and that is the decision rather than a consequence
nobody noticed.** An event stores `template_id` and never a label, so renaming
"Chalk" to "Film Review" re-labels every past chalk session everywhere — list,
event page, public calendar, ICS feed, RSVP page, Monday report — with no row in
`events` rewritten. Brian said so out loud on 2026-09-09 rather than leaving it
implied, and the template editor says it too, above the Name field.

**Colour is a template's own fact, not a guess from its class — LAN-276
correction round 1.** Brian, walking the review environment, 2026-09-10: "In
the template, swatch color should be something that gets chosen, so it gets
added as part of the template." Before this, the calendar coloured a tile by
`event_type`, so every template an operator created showed Practice's blue by
accident — every operator-created template takes `practice`. `event_templates`
gains `colour_key`, a `not null` palette key checked against
`event_templates_colour_key_known`; the check constraint restates the list
`TEMPLATE_COLOUR_PALETTE` (`src/lib/services/event-template-input.ts`) defines,
so the two cannot drift silently. LAN-429 (W2) extended the list with
`lancer_gold` by a forward migration — thirteen keys — and re-toned key `blue`
to Oxford Blue `#002147` in the palette alone, with no migration, so Practice
reads navy on the calendar. The column stores the key rather than a hex
value, so the palette can be re-tuned without a migration. The calendar legend,
every calendar tile (`src/app/calendar/calendar-entry.tsx`,
`src/app/calendar/type-legend.tsx`) and every event list read a template's own
colour now; the seven seeded rows keep exactly the colours the calendar always
...
**Deliberately not implemented, with one partial exception.** Frozen model
§1.2 defers four conceptual entities from the release-one schema (review F14):
**Channel Presence**, **Comms Group** and **Group Membership**, **recognition
counts**, and **durable kit ownership**. The model says they are deferred
"unless a locked workflow pulls them in", and for three of the four none has.
Each is additive when it arrives — all four hang off `people` with no change
to any existing relationship. Kit ownership is the likeliest early pull, since
the subscription invoice prices against it.

**Channel Presence arrived early, in part.** Packet amendment 1 of mission
`M-RECRUITMENT` (Brian, 2026-08-31) makes the sign-up form the mission's single
consent gate and requires an opt-out honoured across every channel — a
deliberate owner amendment, not a quiet migration. `season_messaging_consents`
(LAN-201) is that requirement's landing: season-scoped consent, keyed
`(person_id, season_id)`, checked before every send regardless of channel. What
remains deferred is the rest of the entity — actual channel/group **presence**
(is this person in this season's WhatsApp group), which `W2`'s own open
question leaves unanswered and which no locked workflow yet pulls in.
...
**2026-09-01 — the recruitment schema** (LAN-201, `WP-recruitment-schema`,
...
**2026-09-03 — the onboarding substrate** (LAN-214, `WP-onboarding-substrate`,
mission `M-ONBOARDING-AND-INFORMATION-COMPLETION`, this mission's migration
owner). Seven pieces, in one forward-only migration:

- **`claimed` joined `onboarding_item_status`** — the player's own word,
  awaiting a named human's confirmation (`REQ-item-states`).
- **`onboarding_items_waiver_is_justified` was unwound** to
  `onboarding_items_waiver_author_required` — the author stays mandatory, the
...
- **`onboarding_activity_log`** is the sectioned ask-and-answer log
  `REQ-activity-log` asks for — one row per event, never a count; the same
  append-only posture.
- **`onboarding_agreement_versions`/`onboarding_agreements`** are the
  versioned-agreement mechanism for the Code of Conduct and the photo release
  — version, moment, person; no object storage.
- **`onboarding_chase_settings`** is `W11`'s singleton configuration row —
  first-chase delay, count, interval; the escalation office is read from
  `roles`/`role_assignments`, never a column.
- **`bps_selections`** is item 5 of the item-and-ask inventory, added to the
  roster rather than the checklist (Brian, 2026-09-01).
- **`person_fact_disputes`** was built for the disputed-fact raise-and-resolve
  pair `REQ-no-silent-overwrite` named (Mission 5 deliberately left it
  unbuilt) and then retired by `Q-9` before shipping (Brian, 2026-09-04:
  "I don't think the disputed fact mechanism survives at all"). Present in
  the schema, written by nothing — last-write-wins is the rule now, and the
  ordinary audit trail is the whole of what the record shows.

`PERSON_REFERENCE_COLUMNS`/`PERSON_REFERENCE_COLUMNS_EXCLUDED` in
`person-merge.ts` were extended for the new foreign keys to `people`: five
actor/author columns blindly re-point, and two subjects with per-tuple
...
Protection Privacy Notice verbatim. The `placeholder-v1` row stays: a version an
agreement already points at has to keep resolving to the words that were shown,
which is what the versioned slot is for. `effective_from` is computed one second
after the latest existing photo release version rather than defaulted to
`now()`, because a reset that applied both migrations inside one transaction
would give them the same transaction timestamp and make "the current version"
undecidable.

The body is plain text carrying `[[section]]` markers, one per block the page
renders, and inside a section `## `, `- ` and `1. ` prefixes for a subheading, a
bullet and a numbered clause. Not JSON — the column is `text`, the wording is
the artefact, and a marker is the smallest thing that lets the page put a field
where the paper form has a box while every printed word stays in the row. A body
with no markers parses as one unnamed section of paragraphs, so the Code of
Conduct's placeholder renders exactly as it did until LAN-282 replaces it.

## Audiences by category — LAN-414, LAN-416

State of the App call, 2026-09-22. Stewart defined the club's audiences from the
roster board and Clint confirmed the names, and Brian took the four assignment
columns the roster already keeps and made them audiences an event can be built
from. `20261004090000_audience_categories.sql` is the whole of the change.

**Five categories, one catalogue.** `public.audience_group_category` is
`general`, `coaching`, `warmup`, `special_teams`, `recruits`, in the order the
picker shows them. It is closed for the same reason `public.audience_group` is:
a category is a structural fact about where an audience comes from, not a club
vocabulary anybody edits. One read —
`audienceCategoriesForEventType` in `src/lib/services/audience-selection.ts` —
answers the event form, the template editor, the approval review and the event's
own audience panel (`docs/ux/standards.md` rule 7).

**Why the pair and not more enum values.** LAN-414 left the representation to
the implementer and named the two candidates. The sub-groups are _values of
roster vocabularies the club edits_: LAN-401 renamed position groups on
Stewart's say-so, LAN-409 renamed the braces, and the warmup small groups are
reference data in a table precisely so the list can move. An enum value per
vocabulary value would make every such rename a schema migration and would
couple two closed lists edited by different decisions and different people.
`value text` is also what the roster itself stores those facts as
(`coach_group text`, `position_group text`, `small_group text`), so the audience
row and the assignment row it matches speak the same vocabulary in the same
type. `public.audience_group` therefore does not move, and keeps carrying the
General category, which is genuinely a closed list of system-derived groups
(D43).

**Exactly one of the two, said by a constraint.** Every row on
`event_audience_groups`, `event_template_audience_groups` and (for
`added_by_group`) `event_audience_members` is either a General group — a
non-null `audience_group`, a null `value` — or a category and a non-blank
`value`, never both and never neither. `nulls not distinct` on the natural keys
is load-bearing: with `audience_group` nullable, the ordinary form would treat
two nulls as different and let the same `(event, coaching, 'Quarterbacks')` row
...
the match is the same pure `groupSelectionKeys` every General group already
uses, and the LAN-392 live rule follows for free: an assignment written later
is a roster-board write, the chokepoint runs, and the person is added to every
...
**The cadence stays with the event.** On a Recruitment-type event a recruit
keeps the gentle welcome-and-one-follow-up ladder (REQ-two-ladders,
REQ-never-harsh). On any other type there is no recruit ladder configured at
all, so `scheduleEventLadderIn` treats a recruit as the event's ordinary invitee
and they are invited and chased exactly as a player is for that type. The test
is `event_messaging_plans.recruit_invitation_at`, which is null exactly where
this event was given no recruit ladder; the consent gate on a recruit-capacity
send is unchanged and applies on every type.

## Known deviations from the frozen model

Two, and both are deferrals the model itself directs:

- Three of the four review-F14 entities remain unimplemented: **Comms Group**
  and **Group Membership**, **recognition counts**, and **durable kit
  ownership**. The frozen model marks them schema-deferred; the ticket's
  traceability list names structural coverage. The model governs, and the
  difference is recorded here as required.
- The fourth, **Channel Presence**, arrived early and in part (LAN-201,
  2026-09-01): `season_messaging_consents` is a deliberate owner amendment
  landing one piece of it ahead of the rest, not a reinterpretation of the
  model. What full channel/group presence would still add is unimplemented,
  for the same schema-deferred reason as the other three.
