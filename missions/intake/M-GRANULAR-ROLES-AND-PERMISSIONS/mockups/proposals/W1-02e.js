/* W1-02e — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-02e — Alternative E, compare two seats. Pick a seat and a seat to
   * compare it with; their grants sit side by side, the first editable, the
   * second read. A row where they differ carries a copy-across control, and
   * the head carries the two mass edits: Copy access from <the compared
   * seat> (Copy access from another seat, with the seat already chosen) and
   * Grant everything. Shown: the Treasurer against the Secretary.
   */
  await accessViewHead("2026-27 · compare two seats");
  const ctx = rolesScaffold();
  const LEFT = "Treasurer";
  const RIGHT = "Secretary";
  const a = seed("treasurer");
  const b = seed("full");

  const pickers = el(
    "div",
    `display:flex;gap:${WIDE ? 16 : 0}px;flex-wrap:${WIDE ? "nowrap" : "wrap"}`,
  );
  pickers.innerHTML =
    textField("Seat", LEFT, { select: true, width: WIDE ? "300px" : "100%" }) +
    textField("Compare with", RIGHT, { select: true, width: WIDE ? "300px" : "100%" });
  const top = ctx.section(null);
  top.appendChild(pickers);
  top.appendChild(
    buttonRow([
      appButton("outlined", `Copy access from ${RIGHT}`),
      appButton("outlined", "Grant everything"),
    ]),
  );

  const copyBtn = () =>
    `<span role="button" aria-label="Copy ${RIGHT}'s grant" style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:50%;color:${T.navy}"><svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true" style="fill:${T.navy}"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg></span>`;
  const rowsFor = (kind) =>
    kind === "roster"
      ? ROSTER.map((c) => [c, a.roster[c], b.roster[c], ["None", "View", "Edit"]])
      : kind === "switches"
        ? SWITCHES.map((s) => [
            s,
            a.switches[s] ? "Yes" : "No",
  ...
  [
    ["Roster", "roster"],
    ["Records and adding", "switches"],
    ["Events", "events"],
  ].forEach(([title, kind]) => {
    const s = ctx.section(title);
    const rows = rowsFor(kind);
    if (WIDE) {
      const t = ctx.table(title);
      t.style.tableLayout = "fixed";
      const hr = document.createElement("tr");
      hr.append(
        ctx.th(title === "Records and adding" ? "Switch" : "Category", "width:300px"),
        ctx.th(LEFT, "width:300px"),
        ctx.th("", "width:56px"),
        ctx.th(RIGHT),
      );
      t.tHead.appendChild(hr);
      rows.forEach(([label, left, right, options]) => {
        const differs = left !== right;
        const tr = document.createElement("tr");
        tr.append(
          ctx.td(`<span style="font-weight:${differs ? 600 : 400}">${label}</span>`),
          ctx.td(toggleGroup(options, left)),
          ctx.td(differs ? copyBtn() : "", "text-align:center;padding:0"),
          ctx.td(
            `<span style="font-weight:400;color:${right === "None" || right === "No" ? T.charcoal50 : T.charcoal}">${right}</span>`,
          ),
        );
      ...
      rows.forEach(([label, left, right, options]) => {
        ...
        list.appendChild(
          el(
            "div",
            `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px;display:flex;flex-direction:column;gap:6px`,
            `<div style="font-size:15px;font-weight:600">${label}</div>` +
              `<div style="font-size:12px;color:${T.charcoal70}">${LEFT}</div>${toggleGroup(options, left)}` +
              `<div style="display:flex;justify-content:space-between;align-items:center;font-size:14px;color:${T.charcoal70}"><span>${RIGHT}: <span style="color:${T.charcoal}">${right}</span></span>${differs ? copyBtn() : ""}</div>`,
          ),
        );
      });
      s.appendChild(list);
