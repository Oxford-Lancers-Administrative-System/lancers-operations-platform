...
(async () => {
  ...
  /*
   * W3-03 — one player's record as the Kit Manager: View on Person, Edit on
   * Kit, May open individual records (roster) on, None on everything else.
   * Only the Person and Kit sections exist on the page. Contact and
   * emergency rows, the summary tiles, Onboarding, Activity, Membership,
   * Availability, the assignment groups, Attendance, Other seasons and
   * Status history are absent, not hidden behind a fold. Person's values
   * read without edit links; Kit is open and editable.
   *
   * Photographed as the review account and narrowed by script. Kit arrives
   * folded for this account; it is opened on a copy of its disclosure, so
   * no fold preference is written back.
   */
  signedInAs("Peregrine Oakhanger", ["/operate/roster", "/operate/events"]);
  const main = document.querySelector("main");
  const KEEP = ["person", "kit"];

  /* Every banded roster group outside the grant goes, with the record's own sections. */
  main.querySelectorAll("[data-band]").forEach((n) => {
    if (!KEEP.includes(n.getAttribute("data-band"))) n.remove();
  });
  main.querySelectorAll("details, section").forEach((n) => {
    if (!n.isConnected || n.closest("[data-band]") || n.querySelector("[data-band]")) return;
    const head = n.querySelector("h2, h3");
    if (head) n.remove();
  });

  /* Kit: open a copy, so the live disclosure never reports a toggle. */
  const kit = main.querySelector('[data-band="kit"]');
  if (kit && kit.tagName === "DETAILS") {
    const copy = kit.cloneNode(true);
    copy.open = true;
    const ind = copy.querySelector("[data-disclosure-indicator]");
    if (ind) ind.style.transform = "translateY(3px) rotate(225deg)";
    kit.replaceWith(copy);
  }

  /* The subtitle's entry route and status are Membership, not granted: the season alone remains. */
  Array.from(main.querySelectorAll("p, span, div")).forEach((n) => {
    if (n.children.length === 0 && /^\d{4}-\d{2} membership · /.test(n.textContent.trim()))
      n.textContent = n.textContent.trim().split(" · ")[0];
  });

  /* Summary tiles: Membership, Onboarding, Entry, Books, Constitutional member, Missing data — none granted. */
  const tileText = Array.from(main.querySelectorAll("*")).find(
    (n) => n.children.length === 0 && /Missing required data/.test(n.textContent),
  );
  if (tileText) {
    let grid = tileText.parentElement;
    while (grid && grid !== main && !/Onboarding items/.test(grid.textContent))
      grid = grid.parentElement;
    if (grid && grid !== main) grid.remove();
  }

  /* Person: contact rows belong to Contact & emergency; the person-record link needs People access; values are read, not edited. */
  const person = main.querySelector('[data-band="person"]');
  if (person) {
    const CONTACT = ["Mobile phone", "Personal email", "Emergency contact", "Contactable"];
    Array.from(person.querySelectorAll("*")).forEach((n) => {
      if (!n.isConnected || n.children.length) return;
      if (CONTACT.includes(n.textContent.trim())) {
        let row = n.parentElement;
        while (
          row &&
          row.parentElement &&
          row.parentElement !== person &&
          row.parentElement.children.length < 4
        )
          row = row.parentElement;
        if (row && row !== person) row.remove();
      }
    });
    person.querySelectorAll("a").forEach((a) => {
      if (/person record/i.test(a.textContent)) a.remove();
      else {
    ...
    person.querySelectorAll("button").forEach((b) => {
