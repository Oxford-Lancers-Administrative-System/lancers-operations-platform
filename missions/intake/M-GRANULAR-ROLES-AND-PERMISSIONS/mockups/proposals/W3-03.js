...
(async () => {
   ...
   * Kit, None on everything else). Every section is present, in its place.
   * Person is open and reads as values; Kit is open and editable. Every
   * section the seat holds None on is collapsed and locked: a lock where the
   * chevron was, it cannot be opened, and nothing inside it is sent to the
   * browser. Contact & emergency becomes its own section (today its rows sit
   * inside Person), so it can lock while Person reads. The name stays at the
   * top. The summary tiles are Membership and Onboarding facts, so they go,
   * and the subtitle keeps only the season.
   *
   * Photographed as the review account and narrowed by script. Kit arrives
   * folded for this account; it is opened on a copy of its disclosure, so no
   * fold preference is written back.
   */
  signedInAs("Peregrine Oakhanger", ["/operate/roster", "/operate/events"]);
  const main = document.querySelector("main");

  /* Summary tiles: Membership, Onboarding, Entry, Blues, Constitutional member, Missing data — none granted. */
  ...
  /* Person: its contact rows become Contact & emergency; the rest reads as values. */
  const person = main.querySelector('[data-testid="section-person"]');
  const contact = person.cloneNode(true);
  contact.setAttribute("data-testid", "section-contact-emergency");
  const contactHead = contact.querySelector("summary h2, summary h3");
  if (contactHead) contactHead.textContent = "Contact & emergency";
  const CONTACT = ["Mobile phone", "Personal email", "Emergency contact"];
  Array.from(person.querySelectorAll("*")).forEach((n) => {
    ...
    if (CONTACT.includes(n.textContent.trim())) {
      ...
      while (
        row &&
        row.parentElement &&
        !row.parentElement.matches("details") &&
        row.parentElement.children.length < 4
      )
        row = row.parentElement;
      if (row && !row.matches("details, summary")) row.remove();
    }
  });
  person.querySelectorAll("summary a").forEach((a) => a.remove());
  readOnly(person);
  person.after(contact);
  lockSection(contact);

  /* Kit: open and editable. */
  const kit = main.querySelector('[data-testid="section-kit"]');
  if (kit && !kit.open) openCopy(kit);

  /* Everything else the seat holds None on. */
  [
    "section-onboarding",
    "section-activity",
    "section-season",
