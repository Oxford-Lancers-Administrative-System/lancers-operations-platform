...
(async () => {
  ...
  /*
   * W4-03 — the calendar as the Social Secretary: None on six categories
   * means those events never appear, so only socials are left, drawn in the
   * Social category's colour from W2 (Orange). The legend lists only the
   * categories this seat can see.
   *
  ...
  const [, accent, tint] = CATEGORY_COLOUR.Social;
  const HIDE = ["Chalk", "Game", "Practice", "Strength and conditioning", "Recruitment", "Meeting"];
