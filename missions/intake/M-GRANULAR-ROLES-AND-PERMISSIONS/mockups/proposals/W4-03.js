/* W4-03 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W4-03 — the calendar as the Social Secretary: None on every template but
   * Social means those events never appear, so only socials are left, in the
   * Social template's own colour as today (Orange, read from its editor).
   * The legend lists only the templates this seat can see.
   *
  ...
  const { accent, tint } = paletteOf(
    (TEMPLATES.find((t) => t.name === "Social") || {}).colour || "orange",
  );
  const HIDE = TEMPLATE_NAMES.filter((n) => n !== "Social");
  const main = document.querySelector("main");

  /* Event entries: every link to an event. */
  main.querySelectorAll('a[href^="/operate/events/"]').forEach((a) => {
    const href = a.getAttribute("href");
    if (!/^\/operate\/events\/[0-9a-f-]{36}$/.test(href)) return;
    if (!/Social ·/.test(a.textContent)) {
      a.style.display = "none";
      return;
    }
    a.style.background = tint;
    a.style.borderLeftColor = accent;
    a.querySelectorAll("*").forEach((n) => {
      const cs = getComputedStyle(n);
      if (cs.borderLeftWidth !== "0px") n.style.borderLeftColor = accent;
    });
  });

  /* Legend: keep Social only, in its W2 colour. */
  const walker = document.createTreeWalker(main, NodeFilter.SHOW_TEXT);
  const legend = [];
  while (walker.nextNode()) {
    const n = walker.currentNode;
    const t = n.nodeValue.trim();
    if ((HIDE.includes(t) || t === "Social") && !n.parentElement.closest("a"))
      legend.push([t, n.parentElement]);
  }
  legend.forEach(([t, node]) => {
    const item =
      node.parentElement && node.parentElement.children.length <= 3 ? node.parentElement : node;
    if (t === "Social") {
      const sw = item.firstElementChild;
      if (sw && sw !== node) {
        sw.style.background = tint;
        sw.style.borderColor = accent;
      }
