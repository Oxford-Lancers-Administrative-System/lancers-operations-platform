/* W1-09 — generated from the shared prelude, kit, altb and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * Alternative B — the collapsed matrix. One row per seat; three summary
   * columns, Roster, Events and Records and adding, as counts. A row opens in
   * place to show the seat's grants as chips and the seat's three actions:
   * Open seat (the editor, W1-03), Copy access from another seat, Grant
   * everything. Nothing is edited in the row itself.
   *
   * `open` names the expanded seat; `grantsFor` lets a variant override one
   * seat's grants; `until` marks a seat's time-limited delegation.
   */
  async function drawCollapsedMatrix({ open, grantsFor = {}, until = {}, notice = null }) {
    await accessViewHead();
    const ctx = rolesScaffold();
    if (notice) ctx.section(null).appendChild(successNotice(notice));
    const grantsOf = (seat, profile) => grantsFor[seat] ?? seed(profile);
    const muted = (v) =>
      `<span style="color:${v === "None" ? T.charcoal50 : T.charcoal}">${v}</span>`;

    function detail(seat, profile) {
      const g = grantsOf(seat, profile);
      const locked = profile === "floor";
      const rosterChips = ROSTER.filter((c) => g.roster[c] !== "None").map((c) =>
        chip(`${c} · ${g.roster[c]}`),
      );
      const swChips = SWITCHES.filter((s) => g.switches[s]).map((s) => chip(SWITCH_SHORT[s]));
      const evChips = EVENTS.filter((c) => g.events[c] !== "None").map((c) =>
        chip(`${swatch(c, 10)}&nbsp;${c} · ${g.events[c]}`),
      );
      const line = (label, chips) =>
        `<div style="display:flex;gap:8px 12px;align-items:flex-start;flex-wrap:${WIDE ? "nowrap" : "wrap"};padding:6px 0"><span style="flex:0 0 ${WIDE ? "150px" : "100%"};font-size:12px;letter-spacing:0.08333em;text-transform:uppercase;font-weight:600;color:${T.charcoal70};padding-top:4px">${label}</span><span style="display:flex;flex-wrap:wrap;gap:6px">${chips.length ? chips.join("") : `<span style="font-size:14px;color:${T.charcoal50}">None</span>`}</span></div>`;
      const box = el(
        "div",
        `display:flex;flex-direction:column;gap:4px;padding:${WIDE ? "8px 16px 16px 56px" : "8px 0 0"}`,
        (until[seat]
          ? `<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;padding:6px 0">${chip(`Until ${until[seat]}`, { accent: [T.navy, "rgba(0,33,71,0.08)"] })}${proposedChip()}</div>`
          : "") +
          line("Roster", rosterChips) +
          line("Records and adding", swChips) +
          line("Events", evChips),
      );
      if (!locked) {
        const btns = [appButton("outlined", "Open seat", ctx.hrefFor[seat]), ...massButtons()];
        if (until[seat]) btns.push(appButton("outlined", "End now"));
        box.appendChild(buttonRow(btns, "padding-top:8px"));
      } else {
        box.appendChild(
          buttonRow([appButton("outlined", "Open seat", ctx.hrefFor[seat])], "padding-top:8px"),
        );
      }
      return box;
    }
    ...
    GROUPS.forEach((group) => {
      const s = ctx.section(group.label);
      if (WIDE) {
        const t = ctx.table(group.label);
        t.style.tableLayout = "fixed";
        const hr = document.createElement("tr");
        hr.append(
          ctx.th("", "width:40px;padding-right:0"),
          ctx.th("Role", "width:250px"),
          ctx.th("Roster"),
          ctx.th("Events"),
          ctx.th("Records and adding", "width:170px"),
        );
        t.tHead.appendChild(hr);
        group.seats.forEach(([seat, holder, profile]) => {
          const g = grantsOf(seat, profile);
          const c = counts(g);
          const locked = profile === "floor";
          const isOpen = seat === open;
          const tr = document.createElement("tr");
          tr.append(
            ctx.td(
              `<span style="display:inline-flex;width:28px;height:28px;align-items:center;justify-content:center">${disclosure(isOpen)}</span>`,
              "padding-right:0",
            ),
            ctx.td(
              `<div style="font-weight:600;white-space:nowrap">${seat}</div><div style="font-weight:400;font-size:13px;color:${T.charcoal70};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${holder}</div>${locked ? `<div style="margin-top:4px">${centralRule()}</div>` : ""}`,
            ),
            ctx.td(
              muted(c.roster) +
                (until[seat]
                  ? `<div style="font-size:12px;color:${T.charcoal70}">until ${until[seat]}</div>`
                  : ""),
            ),
            ctx.td(
              muted(c.events) +
            ...
            ctx.td(muted(c.switches)),
          );
          if (isOpen) tr.style.background = "rgba(0,33,71,0.04)";
          t.tBodies[0].appendChild(tr);
          if (isOpen) {
            const dr = document.createElement("tr");
            const cell = ctx.td("", "padding:0;background:rgba(0,33,71,0.04)");
            cell.colSpan = 5;
            cell.appendChild(detail(seat, profile));
            dr.appendChild(cell);
            t.tBodies[0].appendChild(dr);
          }
        });
        s.appendChild(ctx.frame(t));
      } else {
        const list = ctx.cardList.cloneNode(false);
        list.style.cssText = "display:flex;flex-direction:column;gap:12px";
        group.seats.forEach(([seat, holder, profile]) => {
          const c = counts(grantsOf(seat, profile));
          const locked = profile === "floor";
          const isOpen = seat === open;
          const card = el(
            "div",
            `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px;display:flex;flex-direction:column;gap:2px`,
            `<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px"><div style="display:flex;flex-wrap:wrap;gap:4px 8px;align-items:center"><span style="font-size:16px;font-weight:600">${seat}</span>${locked ? centralRule() : ""}</div>${disclosure(isOpen)}</div>` +
              `<div style="font-size:14px;color:${T.charcoal70}">${holder}</div>` +
              `<div style="font-size:14px;color:${T.charcoal70}">Roster: ${c.roster}${until[seat] ? ` · until ${until[seat]}` : ""} · Events: ${c.events} · Records and adding: ${c.switches}</div>`,
          );
          if (isOpen) card.appendChild(detail(seat, profile));
  ...
  /*
   * W1-09 — Proposed for owner approval, not assumed: Alternative B with a
   * time-limited delegation. A club without a Secretary has just granted the
   * Treasurer everything until 31 Oct 2026. On that date the Treasurer's
   * grants return to what they were before, and that return is audited too.
   */
  await drawCollapsedMatrix({
    open: "Treasurer",
    grantsFor: { Treasurer: seed("full") },
    until: { Treasurer: "31 Oct 2026" },
    notice: "Everything granted to the Treasurer until 31 Oct 2026. 23 grants changed.",
  });
})();
