/* W1-02d — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-02d — Alternative D, seat cards. One card per seat: summary chips for
   * Roster and Events, the four switches live on the card, and the seat's
   * actions. The seat page (W1-03) stays the editor for categories.
   * Three cards a row at 1280, one at 375. The Kit Manager shows W1-03's
   * example change.
   */
  await accessViewHead();
  const ctx = rolesScaffold();
  const grantsOf = (seat, profile) => (seat === "Kit Manager" ? seed("kit") : seed(profile));

  GROUPS.forEach((group) => {
    const s = ctx.section(group.label);
    const grid = el(
      "div",
      `display:grid;grid-template-columns:${WIDE ? "repeat(3, minmax(0, 1fr))" : "1fr"};gap:12px`,
    );
    group.seats.forEach(([seat, holder, profile]) => {
      ...
      const card = el(
        "div",
        `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:16px;display:flex;flex-direction:column;gap:8px`,
        `<div style="display:flex;flex-wrap:wrap;gap:4px 8px;align-items:center"><span style="font-size:16px;font-weight:600">${seat}</span>${locked ? centralRule() : ""}</div>` +
          `<div style="font-size:14px;color:${T.charcoal70};margin-top:-6px">${holder}</div>` +
          `<div style="display:flex;flex-wrap:wrap;gap:6px">${chip(`Roster · ${c.roster}`)}${chip(`Events · ${c.events}`)}</div>` +
          `<div style="border-top:1px solid ${T.divider};padding-top:4px">${SWITCHES.map(
            (sw) =>
              `<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;min-height:36px;font-size:13px;color:${T.charcoal}"><span>${sw}</span>${muiSwitch(g.switches[sw], locked)}</div>`,
          ).join("")}</div>`,
      );
      const actions = el(
        "div",
        `display:flex;flex-wrap:wrap;gap:4px;border-top:1px solid ${T.divider};padding-top:8px`,
      );
      actions.appendChild(ctx.textBtn("Open seat", ctx.hrefFor[seat]));
      if (!locked) {
        actions.appendChild(ctx.textBtn("Copy access from another seat"));
        actions.appendChild(ctx.textBtn("Grant everything"));
      }
