/* W4-01 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W4-01 — the events list this term as the Social Secretary, with Manage on
   * the Social template and None on every other template. None means the
   * event never appears, so only socials are left and each bucket's count
   * follows. The Type filter is opened (the running MUI Select, clicked by
   * script) and lists only the templates this seat is granted: Social.
   * Edit templates is absent (managing a template's events is not editing
   * the template — an open question).
   *
   * Photographed as the review account and narrowed by script: there is no
   * Social Secretary login locally.
   */
  signedInAs("Marlowe Fairhurst", ["/operate/events"]);
  const GRANTED = ["Social"];
  const main = document.querySelector("main");
  Array.from(main.querySelectorAll("a")).forEach((a) => {
    if (a.getAttribute("href") === "/operate/events/templates") a.style.display = "none";
  });

  main.querySelectorAll('[data-testid="event-row"]').forEach((row) => {
    const type = row.cells[1] ? row.cells[1].textContent.trim() : "";
    if (!GRANTED.includes(type)) row.remove();
  });
  main.querySelectorAll('[data-testid="event-card"]').forEach((card) => {
    const lines = Array.from(card.querySelectorAll("div.MuiTypography-body2")).map((d) =>
      d.textContent.trim(),
    );
    if (!lines.some((l) => GRANTED.includes(l))) card.remove();
  });
  main.querySelectorAll('[data-testid="event-bucket"]').forEach((bucket) => {
    const count = bucket.querySelectorAll(
      WIDE ? '[data-testid="event-row"]' : '[data-testid="event-card"]',
    ).length;
    if (count === 0) {
      bucket.style.display = "none";
      return;
    }
    const walker = document.createTreeWalker(bucket, NodeFilter.SHOW_TEXT);
    while (walker.nextNode()) {
      const n = walker.currentNode;
      if (/·\s*\d+\s+events?/i.test(n.nodeValue)) {
  ...
  /* Open the Type filter and keep only the granted templates. */
  const typeField = Array.from(main.querySelectorAll(".MuiFormControl-root")).find((fc) => {
    const l = fc.querySelector(".MuiInputLabel-root, label");
    return l && l.textContent.trim() === "Type";
  });
  const trigger = typeField && typeField.querySelector('[role="combobox"], .MuiSelect-select');
  if (trigger && typeField.offsetParent !== null) {
    trigger.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 0 }));
    await new Promise((r) => setTimeout(r, 600));
    document.querySelectorAll('[role="listbox"] [role="option"]').forEach((li) => {
      const t = li.textContent.trim();
      if (TEMPLATE_NAMES.includes(t) && !GRANTED.includes(t)) li.remove();
    });
  }
})();
