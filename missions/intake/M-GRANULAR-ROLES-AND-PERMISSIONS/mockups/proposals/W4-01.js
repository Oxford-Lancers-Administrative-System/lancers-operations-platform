/* W4-01 — generated from the shared prelude and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W4-01 — the events list this term as a Social Secretary with Manage on
   * Social and None on the other six. None means the event never appears, so
   * only the socials are left; the counts in each bucket follow.
   *
   * Photographed signed in as the review account and narrowed by this script,
   * because there is no Social Secretary login locally.
   */
  signedInAs("Marlowe Fairhurst", ["/operate/events"]);

  const KEEP = ["Social"];
  const templates = Array.from(document.querySelectorAll("a")).find(
    (a) => a.getAttribute("href") === "/operate/events/templates",
  );
  if (templates) templates.style.display = "none";

  document.querySelectorAll('[data-testid="event-row"]').forEach((row) => {
    const type = row.cells[1] ? row.cells[1].textContent.trim() : "";
    if (!KEEP.includes(type)) row.remove();
  });
  document.querySelectorAll('[data-testid="event-card"]').forEach((card) => {
    ...
    if (!lines.some((l) => KEEP.includes(l))) card.remove();
  });
  document.querySelectorAll('[data-testid="event-bucket"]').forEach((bucket) => {
