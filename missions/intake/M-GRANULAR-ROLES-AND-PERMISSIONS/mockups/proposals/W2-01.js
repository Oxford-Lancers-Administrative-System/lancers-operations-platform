/* W2-01 — generated from the shared prelude and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W2-01 — Template categories at the top of the event templates page.
   *
   * Seven fixed rows, one colour control each, nothing else: no add, no rename,
   * no delete (Brian, 2026-09-25). The colours shown are the ones the seven
   * seeded templates carry today (migration 20260916090000), moved from the
   * template to its category. Each template row then names its category, with
   * the category's swatch; the template itself no longer has a colour.
   */
  (() => {
    const TEMPLATE_CATEGORY = Object.fromEntries(EVENTS.map((c) => [c, c]));
    const stack = document.querySelector('[data-testid="event-templates"]');
    const header = stack.querySelector('[data-testid="page-header"]');
    const table = document.querySelector('[data-testid="template-table"]');
    const paper = table.closest(".MuiPaper-root");
    const desktopBox = paper.parentElement;
    const cardList = document.querySelector('[data-testid="template-cards"]');
    const protoTh = table.querySelector("th");
    const protoTd = table.querySelector("td");
    const wide = window.innerWidth >= 900;

    const heading = (text) =>
      el(
        "h2",
        `margin:0;font-size:1rem;font-weight:700;line-height:1.75;color:${T.charcoal}`,
        text,
      );
    const colourField = (cat) => {
      const [label] = CATEGORY_COLOUR[cat];
      return `<span style="display:inline-flex;align-items:center;justify-content:space-between;gap:10px;min-width:150px;min-height:40px;padding:0 12px;border:1px solid ${T.outline};border-radius:8px;background:#fff;font-size:14px;color:${T.charcoal}"><span style="display:inline-flex;align-items:center;gap:8px">${swatch(cat)}${label}</span><svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true" style="fill:${T.charcoal70}"><path d="M7 10l5 5 5-5z"/></svg></span>`;
    };

    /* The categories section. */
    const section = el("section", "display:flex;flex-direction:column;gap:8px");
    section.appendChild(heading("Template categories"));
    if (wide) {
      const t = document.createElement("table");
      t.className = table.className;
      t.setAttribute("aria-label", "Template categories");
      const cell = (proto, html, css = "") => {
        const c = proto.cloneNode(false);
        c.innerHTML = html;
        c.style.cssText = css;
        return c;
      };
      ...
      ["Category", "Colour", "Templates"].forEach((h) => hr.appendChild(cell(protoTh, h)));
      head.appendChild(hr);
      const body = document.createElement("tbody");
      EVENTS.forEach((cat) => {
        const tr = document.createElement("tr");
        tr.appendChild(cell(protoTd, `<span style="font-weight:600">${cat}</span>`, "width:40%"));
        tr.appendChild(cell(protoTd, colourField(cat), "padding:6px 12px"));
        tr.appendChild(cell(protoTd, "1"));
        body.appendChild(tr);
      });
      t.append(head, body);
      const box = desktopBox.cloneNode(false);
      const p = paper.cloneNode(false);
      p.appendChild(t);
      box.appendChild(p);
      ...
      EVENTS.forEach((cat) =>
        list.appendChild(
          el(
            "div",
            `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;gap:12px`,
            `<div><p style="margin:0;font-size:16px;font-weight:600;color:${T.charcoal}">${cat}</p><div style="font-size:14px;color:${T.charcoal70}">1 template</div></div>${colourField(cat)}`,
          ),
        ),
      );
    ...
    section.after(heading("Templates"));

    /* Template rows: a Category column in place of any per-template colour. */
    const headRow = table.tHead.rows[0];
    const catHead = headRow.cells[0].cloneNode(false);
    catHead.textContent = "Category";
    headRow.cells[0].after(catHead);
    Array.from(table.tBodies[0].rows).forEach((row) => {
      const name = row.cells[0].textContent.trim();
      const cat = TEMPLATE_CATEGORY[name] ?? "Practice";
      const c = row.cells[1].cloneNode(false);
      c.innerHTML = `<span style="display:inline-flex;align-items:center;gap:8px;white-space:nowrap">${swatch(cat)}${cat}</span>`;
      row.cells[0].after(c);
    });
    cardList.querySelectorAll('[data-testid="template-card"]').forEach((card) => {
      const name = card.querySelector(".MuiTypography-subtitle1").textContent.trim();
      const cat = TEMPLATE_CATEGORY[name] ?? "Practice";
      const facts = card.querySelector('[data-testid="template-card-facts"]');
      if (facts)
        facts.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px">${swatch(cat, 12)}Category ${cat}</span> · ${facts.textContent}`;
    });
  })();
})();
