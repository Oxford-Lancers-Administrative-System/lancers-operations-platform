/* W1-02a — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-02a — Alternative A, category first. The Access view asks for one
   * category, and every seat's level for that category is one list, changed
   * in place. Shown on Kit, with W1-03's example change applied (Kit Manager:
   * Edit). The Category select lists the twelve roster categories, the four
   * switches and the seven event categories, grouped.
   *
   * The mass edits act on a seat, not a category, so they sit above the list
   * and ask for the seat in their own dialog.
   */
  await accessViewHead();
  const ctx = rolesScaffold();
  const CATEGORY = "Kit";
  const levelFor = (profile, seat) =>
    seat === "Kit Manager" ? "Edit" : seed(profile).roster[CATEGORY];

  const tools = el("div", "display:flex;flex-direction:column;gap:8px");
  tools.innerHTML = textField(
    "Category",
    `<span style="display:inline-flex;align-items:center;gap:8px">${CATEGORY}<span style="color:${T.charcoal70};font-size:14px">· Roster</span></span>`,
    { select: true, width: WIDE ? "360px" : "100%" },
  );
  tools.appendChild(buttonRow(massButtons()));
  const head = ctx.section(null);
  head.appendChild(tools);

  GROUPS.forEach((group) => {
    const s = ctx.section(group.label);
    if (WIDE) {
      const t = ctx.table(`${group.label} — ${CATEGORY}`);
      const hr = document.createElement("tr");
      hr.append(
        ctx.th("Role", "width:240px"),
        ctx.th("Current holder"),
        ctx.th(`${CATEGORY}`, "width:260px"),
      );
      ...
      group.seats.forEach(([seat, holder, profile]) => {
        ...
        tr.append(
          ctx.td(
            `<div style="font-weight:600;white-space:nowrap">${seat}</div>${locked ? `<div style="margin-top:4px">${centralRule()}</div>` : ""}`,
          ),
          ctx.td(
            `<span style="font-size:14px;font-weight:400;color:${holder === "Not assigned" ? T.charcoal70 : T.charcoal}">${holder}</span>`,
          ),
          ctx.td(
            toggleGroup(["None", "View", "Edit"], levelFor(profile, seat), { disabled: locked }),
          ),
        );
      ...
      group.seats.forEach(([seat, holder, profile]) => {
        ...
        list.appendChild(
          el(
            "div",
            `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px;display:flex;flex-direction:column;gap:8px`,
            `<div style="display:flex;flex-wrap:wrap;gap:4px 8px;align-items:center"><span style="font-size:16px;font-weight:600">${seat}</span>${locked ? centralRule() : ""}</div><div style="font-size:14px;color:${T.charcoal70}">${holder}</div>${toggleGroup(["None", "View", "Edit"], levelFor(profile, seat), { disabled: locked })}`,
          ),
        );
      });
      s.appendChild(list);
