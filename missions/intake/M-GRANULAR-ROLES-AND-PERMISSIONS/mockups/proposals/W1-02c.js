/* W1-02c — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-02c — Alternative C, presets. A seat receives a named bundle and is
   * then adjusted per category on its own page (W1-03). Four presets, as
   * Brian named them: Everything, Coach standard, Read-only, and Same as
   * <another seat>. "Same as" is Copy access from another seat; "Everything"
   * is Grant everything. A seat whose grants match no preset reads Custom,
   * with how it differs from the nearest.
   *
   * Shown: the seeds as they stand (so most seats read Custom on day one,
   * which is the honest cost of this alternative), with the Head Coach's
   * preset menu open.
   */
  await accessViewHead();
  const ctx = rolesScaffold();
  const PRESETS = {
    Everything: seed("full"),
    "Coach standard": {
      ...
      switches: Object.fromEntries(SWITCHES.map((s) => [s, false])),
      events: Object.fromEntries(EVENTS.map((c) => [c, "None"])),
    },
    "Read-only": {
      roster: Object.fromEntries(ROSTER.map((c) => [c, "View"])),
      switches: Object.fromEntries(SWITCHES.map((s) => [s, false])),
      events: Object.fromEntries(EVENTS.map((c) => [c, "View"])),
    },
  };
  function presetOf(g) {
    for (const [name, p] of Object.entries(PRESETS))
      if (grantDiff(p, g).length === 0) return [name, 0];
    let best = null;
    for (const [name, p] of Object.entries(PRESETS)) {
      const n = grantDiff(p, g).length;
      if (!best || n < best[1]) best = [name, n];
    }
    return ["Custom", best];
  }
  const OPEN = "Head Coach";

  /* What each preset grants, as a reference table above the seats. */
  const ref = ctx.section("Presets");
  ref
    .querySelector("h2")
    .insertAdjacentHTML(
      "beforeend",
      ` <span style="margin-left:8px">${proposedChip("Proposed · new concept")}</span>`,
    );
  const refRows = [
    ["Everything", "Edit all · 4 of 4 · Manage all"],
    [
      "Coach standard",
      "View Person · Edit Availability and the five assignment groups · no events",
    ],
    ["Read-only", "View all · none of the four · View all events"],
    ["Same as …", "Copies another seat's grants, with no end date"],
  ];
  if (WIDE) {
    const t = ctx.table("Presets");
    const hr = document.createElement("tr");
    hr.append(ctx.th("Preset", "width:250px"), ctx.th("Grants"));
    t.tHead.appendChild(hr);
    refRows.forEach(([n, d]) => {
      const tr = document.createElement("tr");
      tr.append(ctx.td(n), ctx.td(`<span style="font-weight:400">${d}</span>`));
      t.tBodies[0].appendChild(tr);
    });
    ref.appendChild(ctx.frame(t));
  } else {
    ...
    refRows.forEach(([n, d]) =>
      list.appendChild(
        el(
          "div",
          `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px`,
          `<div style="font-weight:600;font-size:15px">${n}</div><div style="font-size:14px;color:${T.charcoal70}">${d}</div>`,
        ),
      ),
    );
  ...
  const menu = () =>
    el(
      "div",
      `position:absolute;z-index:20;${WIDE ? "top:44px;left:0" : "top:52px;left:0;right:0"};min-width:260px;background:#fff;border-radius:8px;box-shadow:0 5px 5px -3px rgba(0,0,0,.2),0 8px 10px 1px rgba(0,0,0,.14),0 3px 14px 2px rgba(0,0,0,.12);padding:8px 0;font-size:15px`,
      ["Everything", "Coach standard", "Read-only", "Same as …", "Custom"]
        .map(
          (o) =>
            `<div style="display:flex;justify-content:space-between;align-items:center;min-height:40px;padding:6px 16px;${o === "Custom" ? "background:rgba(0,33,71,0.08)" : ""}">${o}${o === "Same as …" ? `<span style="color:${T.charcoal70}">›</span>` : ""}</div>`,
        )
        .join(""),
    );
  const presetSelect = (name, locked, open) => {
    const wrap = el("div", `position:relative;${WIDE ? "width:220px" : "width:100%"}`);
    wrap.innerHTML = `<div style="border:1px solid ${open ? T.navy : T.outline};border-radius:8px;min-height:40px;display:flex;align-items:center;justify-content:space-between;padding:0 8px 0 12px;font-size:15px;font-weight:400;color:${locked ? T.charcoal50 : T.charcoal};${locked ? "background:" + T.ground : "background:#fff"}"><span>${name}</span><svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true" style="fill:${T.charcoal70}"><path d="M7 10l5 5 5-5z"/></svg></div>`;
    if (open) wrap.appendChild(menu());
    return wrap;
  };

  GROUPS.forEach((group) => {
    const s = ctx.section(group.label);
    const rows = group.seats.map(([seat, holder, profile]) => {
      const g = seed(profile);
      const [name, near] = presetOf(g);
      const diff =
        name === "Custom"
          ? `${near[1]} ${near[1] === 1 ? "difference" : "differences"} from ${near[0]}`
          : "—";
      return { seat, holder, profile, name, diff, locked: profile === "floor" };
    });
    if (WIDE) {
      ...
      rows.forEach((r) => {
        const tr = document.createElement("tr");
        const presetCell = ctx.td("", "overflow:visible");
        presetCell.appendChild(presetSelect(r.name, r.locked, r.seat === OPEN));
        tr.append(
          ctx.td(
            `<div style="font-weight:600;white-space:nowrap">${r.seat}</div><div style="font-weight:400;font-size:13px;color:${T.charcoal70}">${r.holder}</div>${r.locked ? `<div style="margin-top:4px">${centralRule()}</div>` : ""}`,
          ),
          presetCell,
          ctx.td(
            `<span style="font-weight:400;color:${r.diff === "—" ? T.charcoal50 : T.charcoal70}">${r.diff}</span>`,
          ),
          ctx.td("", "text-align:right"),
        );
        tr.cells[3].appendChild(ctx.textBtn("Open seat", ctx.hrefFor[r.seat]));
        t.tBodies[0].appendChild(tr);
      });
      const frame = ctx.frame(t);
      frame
        .querySelectorAll("*")
        .forEach((n) => n.classList.contains("MuiPaper-root") && (n.style.overflow = "visible"));
      frame.firstElementChild.style.overflow = "visible";
      s.appendChild(frame);
    } else {
      const list = ctx.cardList.cloneNode(false);
      ...
      rows.forEach((r) => {
        const card = el(
          "div",
          `background:#fff;border:1px solid ${T.divider};border-radius:8px;padding:12px 16px;display:flex;flex-direction:column;gap:8px;position:relative`,
          `<div style="display:flex;flex-wrap:wrap;gap:4px 8px;align-items:center"><span style="font-size:16px;font-weight:600">${r.seat}</span>${r.locked ? centralRule() : ""}</div><div style="font-size:14px;color:${T.charcoal70}">${r.holder}${r.diff === "—" ? "" : " · " + r.diff}</div>`,
        );
        card.appendChild(presetSelect(r.name, r.locked, r.seat === OPEN));
