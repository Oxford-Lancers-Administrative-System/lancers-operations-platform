...
(async () => {
  ...
  /*
   * W3-02 — the roster board as a coach (the Head Coach), granted as an
   * example, not a seed: View on Person; Edit on Availability and the five
   * assignment groups (Coaching, Offensive, Defensive, Special teams, Warmup);
   * None on Onboarding, Membership, Kit and Contact & emergency; May add to
   * the roster off. Only granted groups' columns exist; Add players is absent.
   * Contactable stays in Person and Missing stays where it is, as on main.
   * A player's name opens the record: records open for anyone who can reach
   * the roster.
   *
   * Photographed as the review account and narrowed by script. Special teams
   * and Warmup stay folded, as this account's stored preference has them.
   */
  ...
  /* Status is Membership, Missing onboarding data is Onboarding: not granted. Availability stays. */
  ...
  if (live && WIDE) {
    const { copy, shown: n } = narrowBoard(live, {
      granted: [
        "person",
        "availability",
        "coaching",
    ...
    shown = n;
    await scrollBoardTo(copy, "band-toggle-availability", 560);
  }

  /* Phone cards lose the Membership status chip; the missing flag and Call stay with Person. */
  ...
  /* The phone draws cards, not the table; it reads the count measured on the desktop board. */
  if (label) label.textContent = label.textContent.replace(/\d+ columns/, `${shown || 16} columns`);
})();
