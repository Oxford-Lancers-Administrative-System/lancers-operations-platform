/* W1-07 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-07 — Copy access from another seat, on the Treasurer's seat page (the
   * Treasurer starts with None everywhere). The copy asks which seat, lists
   * every grant that would change, and has no end date. Grant everything is
   * the same dialog without the From field. Both are Proposed for owner approval.
   */
  await appButtons();
  const before = seed("treasurer");
  const from = seed("full");
  accessSection({ grants: before, actions: true });
