/* W1-07 — generated from the shared prelude, kit, seatkit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-07 — the mass edit, step one: Copy access from another seat, on the
   * Treasurer's seat page. The two mass edits sit at the top of the Access
   * section; the copy asks which seat, shows every grant that would change,
   * and has no end date. (Grant everything is the same dialog without the
   * seat field.) Every alternative in W1-02a–e opens this dialog.
   */
  await appButtons();
  const before = seed("treasurer");
  const from = seed("full");
  accessSection({ grants: before, actions: buttonRow(massButtons(), "margin-bottom:8px") });
  const lines = grantDiff(before, from);
  const cancel = appButton("outlined", "Cancel");
  cancel.style.border = "none";
  dialog(
    "Copy access from another seat",
    textField("From", "Vice-President", { select: true }) +
      `<div style="font-size:12px;letter-spacing:0.08333em;text-transform:uppercase;font-weight:600;color:${T.charcoal70};margin:8px 0 4px">Treasurer · ${lines.length} changes</div>` +
      `<ul style="margin:0;padding:8px 8px 8px 28px;max-height:${PHONE ? 300 : 260}px;overflow:auto;border:1px solid ${T.divider};border-radius:8px;line-height:1.7">${lines.map((l) => `<li>${l}</li>`).join("")}</ul>`,
    [cancel, appButton("create", "Copy access")],
  );
})();
