/* W3-04 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W3-04 — the recruitment board as a seat with View on Recruits only (the
   * Gameday Secretary in this example) and May open individual records
   * (recruits) off, May add recruits off. The board reads; nothing edits:
   * no QR code, no Add recruit, every editable cell drawn as its value, and
   * a prospect's name is plain text, so a row opens nothing.
   *
  ...
  main
    ...
    .forEach((b) => b.remove());
  const plain = (node, text, css = "") => {
    const span = document.createElement("span");
    span.textContent = text;
    span.style.cssText = `font-size:14px;font-weight:400;color:${/^Not recorded$|^—$/.test(text) ? T.charcoal50 : T.charcoal};${css}`;
    node.replaceWith(span);
  };
  main.querySelectorAll('a[href^="/operate/recruitment/"]').forEach((a) => {
    const href = a.getAttribute("href");
    if (/^\/operate\/recruitment\/[0-9a-f-]{36}$/.test(href)) {
      if (a.dataset.testid === "recruitment-card-open") {
        a.removeAttribute("href");
        a.style.cursor = "default";
      } else plain(a, a.textContent.trim(), "font-weight:600");
    }
  });
  main.querySelectorAll("table .MuiInputBase-root, table .MuiSelect-root").forEach((sel) => {
    if (!sel.isConnected) return;
    const shown = sel.querySelector(".MuiSelect-select") || sel;
    plain(sel.closest(".MuiFormControl-root") || sel, shown.textContent.trim() || "—");
  });
  main.querySelectorAll(".MuiTypography-caption").forEach((c) => {
    if (/edit on the record|edit here/.test(c.textContent)) c.textContent = "view";
  });
  main.querySelectorAll("table button, table [role=button]").forEach((b) => {
    if (b.closest("thead")) return;
    plain(b, b.textContent.trim() || "—");
  });
})();
