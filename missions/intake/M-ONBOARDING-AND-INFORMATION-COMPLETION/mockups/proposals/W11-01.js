(async () => {
  ...
  const fillOnboardingSection = ({ label, fields, button }) => {
    ...
    // Onboarding belongs directly below Recruitment and above Event messaging —
    // the two person-lifecycle chases together, then the events. The shipped page
    // renders it last, so the proposal moves it.
    const recruitment = must(
      $('[data-testid="recruitment-cycle-section"]'),
      "the page has no recruitment section to sit below",
    );
    recruitment.after(section);
