...
const WORKFLOWS = [
  ...
  },
  {
    ...
    grounding: "photograph",
    lede: `The player gets one message and one link, and behind that link is a short sequence: their
      details, then the Code of Conduct, then the photo release, then BUCS Play and Hudl. One link,
      five steps, one open ask. This is the mission's largest workflow and its only one whose actor
      is not an operator.`,
    legend: [
      "<strong>Split into pages on Brian's direction, 2026-09-01.</strong> The Code of Conduct, the photo release and the BUCS Play instructions are each their own page now, not a tick on the form: \"the code of conduct needs to be its own page… We scroll to the bottom, and it says, 'Click I agree to the code of conduct'… You go to the next page\"",
      ...
      "<strong>The link is already on <code>main</code>, and it is not new.</strong> <code>person_access_tokens</code> ships one live durable credential per person per season, enforced by a partial unique index — which is <em>exactly</em> <code>T11-one-request</code>'s \"one open ask, ever\". Five pages, still one link",
      "<strong>There is no way to store a document or take a signature.</strong> No storage bucket is configured, no table holds a document or a blob, and the only file input in the application is the event CSV import, which parses in memory and stores nothing. <strong>Both are new substrate</strong>, and the e-signature question is on W4-04",
      "<strong>Nothing gates.</strong> A player may finish the sequence having filled in nothing at all. The gaps simply stay outstanding and stay chased",
      "<strong>Three doors, one sequence — with one visible difference.</strong> A flipped recruit sees no consent step, because <code>season_messaging_consents</code> is unique per person per season and theirs already says granted. W4-02 is that difference",
    ],
    screens: [
      P(
        "W4-01",
        ...
        `Merrick Thornbury arrived in this season's import. His record really does hold his name,
         mobile, personal email and a full emergency contact, and really is missing college,
         matriculation year, expected graduation, degree field and date of birth. <strong>The
         emergency contact is now five fields</strong>, matching what the database actually stores.`,
        [
          "<strong>The strip is now the map of the sequence</strong> — five steps behind one link, and where this person is in it. Subscriptions, kit, the squad photo and the comms groups are the club's to tick on the roster board and never appear here",
          "<strong>Consent, and it is still the first thing asked.</strong> Unticked, because this is the moment it is asked",
          ...
          "<strong>The emergency contact, as five fields.</strong> <code>person_emergency_contacts</code> stores first name, last name, relationship, phone and email. <strong>Relationship is the fifth</strong>, not in Brian's list — it is shown so he can keep it or drop it; the table is happy either way",
        ],
        "oxfordlancers.example/me/[token]/details",
      ),
      P(
        "W4-02",
        "Step 1 for a flipped recruit, and the one visible difference",
        ...
        [
          ...
          "<strong>The facts recruitment never asks anybody for</strong> — expected graduation, degree field, date of birth",
      ...
      P(
        "W4-03",
        "Step 3 — the Code of Conduct, on its own page",
        `Brian's direction, built. The document is on the page, it scrolls, and agreeing is only
         possible from the end of it. The pane is shown scrolled to its end because the end is where
         the mechanic lives. <strong>The words are placeholder and say so</strong> — the real text is
         Clint's through Task 07, and there is nowhere on <code>main</code> to put it.`,
        [
      ...
      P(
        "W4-04",
        "Step 4 — the photo release, and the signature question",
        `Same shape as the Code of Conduct, deliberately. Brian: "Photo release should also be there
         and should also be a signed document, I think. I don't know. Do we have a way to handle
         signed documents right now in the thing? I don't think so." <strong>He is right — there is
         none.</strong>`,
        [
          ...
          "<strong>OPEN DECISION.</strong> A true signature — drawn, or a signed PDF — needs object storage and a signature control, and the application has neither. Recording the version, the moment and the person needs no new infrastructure. <strong>Recommended: the second</strong>, with e-signature additive if Brian wants it later",
        ],
        "oxfordlancers.example/me/[token]/photo-release",
      ),
      P(
        "W4-05",
        "Step 5 — BUCS Play as a set of steps, and Hudl alongside it",
        `Brian: "Bucs play should be a set of steps. Again, that's its own page as well." Built.
         <strong>The steps are placeholder and say so</strong> — Stewart described this ask on
         2026-08-11 and Task 10 deferred the copy to this mission; nobody has written it.`,
        [
          "<strong>The club records only what the player says.</strong> Nothing the player does on BUCS Play is visible to this system",
          ...
          "<strong>Hudl rides along here rather than taking a sixth page</strong> — it has no document and no steps of its own, and its first half is the club's job. <strong>Brian was undecided on this one</strong>; splitting it out is one line of change",
        ],
        "oxfordlancers.example/me/[token]/bucs-play",
      ),
      P(
        "W4-06",
        "Done — what was saved, and what is still outstanding",
        `The ask moves <code>opened → submitted</code>. The page does not pretend the record is
         finished: Merrick left his degree field blank and has not registered on BUCS Play, which
         <code>R3-G</code> expressly permits.`,
        [
          "<strong>What moved, and what did not.</strong> Finishing the sequence with things outstanding is a normal outcome, not a failure state",
          "<strong>The same link, still open.</strong> Anything outstanding is asked for on this URL. A second ask is never created alongside it — <code>person_access_tokens</code>' own index makes that impossible",
        ],
        "oxfordlancers.example/me/[token]/details",
      ),
      P(
        ...
        `The shipped answer link already has this state and its own words for it. This reuses that
         shape rather than inventing a second way to say the same thing: the heading states the fact,
         one line says what to do if something changes, and there is no sequence to re-enter.`,
        [
          "<strong>Everything the player themselves owns is done</strong>, and the page says so rather than showing an empty form",
          "<strong>The club still has items outstanding against this person.</strong> None of them is the player's, so none appears on the player's page — the operator-owned half lives on W6's record",
        ],
        "oxfordlancers.example/me/[token]/details",
      ),
      P(
        ...
        [
          "<strong>The one sentence that changes.</strong> The shipped body says \"If the event has already started, response changes are closed\" — the answer link's business, and untrue of a collection link",
