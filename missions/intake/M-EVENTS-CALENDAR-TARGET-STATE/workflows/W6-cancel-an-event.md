...
| **Whether "cancelled" is the right word, or whether a "paused" state should exist**    | `proposed for owner approval` | Brian, 2026-08-21: "I'm not sure if it would say cancelled. Maybe pause would be better." **This is not a wording change.** `cancelled` is one of three stored statuses under D27 and is terminal under D60; a paused state is a fourth status that can be left, which is a change to the approved domain model and to release scope. **Recommended: keep `cancelled`** — a postponed event is a cancellation plus a new event, which needs no new state and cannot leave people holding a yes for something that may never happen | **Open — Brian** |
