...
renders "—" until a register has been saved** (D73, D74) — an event
...
**One season is open, and it is the only one this mission knows about.** Every
surface here — the list, both calendars, the event pages, the feed, the import
screen — operates inside it. There is no season selector, no way to navigate to
another season, and nothing anywhere qualifies itself by which season it means.

Brian, 2026-08-21: _"There is no reason why you would ever have to switch a
calendar. Everything in this season is happening within this season … It's just
supposed to be one season open at a time."_

- **A season's window opens in the Long Vacation before Michaelmas**, at the
  AGM, and runs to the following Long Vacation — so the calendar spans Long
  Vacation → Michaelmas → Christmas Vacation → Hilary → Easter Vacation →
  Trinity → Long Vacation.
- **An event belongs to exactly one season**, which the schema already enforces
  through a non-null `season_id`.
- **Changing which season is open is not this mission's.** It belongs to
  whatever mission handles the season lifecycle, and until that exists this is
  simply the rule.
...
- Reference boundaries MT26, HT27 and TT27 place correctly, including a
  ...
  from heading text.
- Every surface reads the open season, and no surface offers a way to reach a
  different one.
...
| **There is one open season and no way to switch.** Every surface reads it; nothing qualifies itself by season                                                                                                  | `locked`                    | Brian, 2026-08-21: "There is no reason why you would ever have to switch a calendar … It's just supposed to be one season open at a time." Changing the open season belongs to a season-lifecycle mission that does not yet exist                                                                                                                                                                                                                                                                                        | Settled                     |
| The club-link tier survives as `D2` approved it                                                                                                                                                                | `locked`                    | Closed at `W7` on 2026-08-21: coaches hold no operator account, so without the middle tier a coach cannot see who is coming to their own session. Delivery stays the only operator-locked element                                                                                                                                                                                                                                                                                                                        | Settled — `W7`              |
