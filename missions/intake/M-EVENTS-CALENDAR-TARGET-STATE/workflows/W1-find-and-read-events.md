...
- **It breaks out by period** — this week, this month, and the longer period —
  with grouped periods rendering as discrete tables rather than one flat run
  (D84). See the open question about the longest bucket below.
...
- **Two open seasons are two calendars, not one merged view.** The displayed
  window of consecutive seasons overlaps in the Long Vacation; ownership does
  not. See the open question below.
...
| **The longest list bucket — quarter or term?**                                                                                                            | `proposed for owner approval` | Brian said "this month, this quarter, this week, whatever"; D84 says flat / by week / by month / **by term**. Recommended default: **term**, because the club runs on terms, the list already carries a Term + Week column, and a calendar quarter cuts Michaelmas in half                                           | **Open — Brian** |
| **How two open seasons share the Long Vacation**                                                                                                          | `proposed for owner approval` | 2026–27 ends in Long Vacation 2027 and 2027–28 opens at an AGM inside it. Recommended default: the _displayed window_ of consecutive seasons overlaps there, while _ownership_ does not — each event sits in exactly one season, and each season's calendar shows only its own. The season-open date is the boundary | **Open — Brian** |
