...
**On a row that updates an existing event, an empty cell changes nothing.**
Brian, 2026-08-21: _"A blank field on a CSV means no change. This includes other
types of white space or anything like that. If it's blank or has white space, it
means no change. Only if it has non-white space does it then change."_

- **Whitespace is blank.** A cell containing spaces, tabs or a non-breaking
  space is empty, not a value.
- **Only non-whitespace content changes a value.** There is therefore no way to
  _clear_ a field through an import, which is the right trade: a spreadsheet
  round trip drops trailing values far more often than anybody deliberately
  empties one, and clearing a field on the event itself takes one edit.
- **On a row that creates a new event**, there is nothing to leave alone, so a
  blank cell simply means the field is unset — and the type's template default
  applies, exactly as it does for any other new draft (`W8`).
...
- A row whose cells are blank or whitespace-only changes nothing on the event it
  matches, and creating a row with blank cells takes the template's defaults.
...
| **A blank or whitespace-only cell on an update row changes nothing; only non-whitespace content changes a value. On a new row it means unset, and the template default applies** | `locked`                    | Brian, 2026-08-21. There is deliberately no way to clear a field by import                                                                                                                                                                                                                                                       | Settled        |
...
| Bulk delete does not exist. D35 is retired for Release One                                                                                                                       | `locked`                    | Closed at `W4` on 2026-08-21 — Brian: "The bulk delete decision: yeah, retire it." This workflow's upsert corrects a wrong batch without removing anything                                                                                                                                                                       | Settled — `W4` |
