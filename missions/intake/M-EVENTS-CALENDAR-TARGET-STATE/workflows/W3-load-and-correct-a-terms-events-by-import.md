...
| Import and export live under **Create event**, which becomes a menu — Add one event · Import from a CSV · Export this season | `locked`                      | Decided 2026-08-21 after Brian judged the first attempt ungrounded: "If I want to bulk import, it should be create event." Importing is a way of creating events, so it belongs where creating them already is, rather than as a third and fourth button competing in the header. Export sits with it because it is the same round trip read backwards. The Events header therefore keeps exactly the two controls `W1` approved | Settled         |
