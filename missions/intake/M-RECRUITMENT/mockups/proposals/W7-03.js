(async () => {
  ...
  const pageButton = (text) => {
    ...
    must(
      row.querySelector('[data-intake-action-group="1"]'),
      "the action row lost its group",
    ).append(button);
    return button;
  };
  ...
  //
  // BOTH answers from W7-02 land here. "Yes, that's me" adds nothing and comes
  // straight through; "No, I'm new" creates the person first. Either way the
  // recruit sees one page and one way into the group, and nothing reads as an
  // error — Brian, 2026-08-31: "If they say yes, it says 'Great,' and then pops
  // them to the WhatsApp board. If it's no, it creates it as a new thing."
  captureFormControls();
