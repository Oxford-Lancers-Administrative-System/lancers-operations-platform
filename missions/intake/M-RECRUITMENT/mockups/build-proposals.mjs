...
for (const file of readdirSync(SRC).sort()) {
  ...
  // Formatted on the way out. `prettier --check .` covers generated files too,
  // so a generator that emits unformatted output breaks CI the next time it is
  // run — which it did, on this mission's own first pull request.
  writeFileSync(
    target,
    await format(
