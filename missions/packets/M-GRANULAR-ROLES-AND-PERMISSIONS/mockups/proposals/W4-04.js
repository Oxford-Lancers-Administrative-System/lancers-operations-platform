/* W4-04 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W4-04 — the Follow-ups queue (read here as "the delivery board": the one
   * surface that lists invitations across events) as the Social Secretary.
   * Rows for events of any template but Social never appear; the heading counts follow.
   *
   * Which events are social is read from the running events list, not typed
   * in. Photographed as the review account and narrowed by script.
   */
  signedInAs("Marlowe Fairhurst", [
    "/operate/events",
    "/operate/admin/follow-ups",
    "/operate/admin/messaging",
  ]);
  const html = await (
    await fetch("/operate/events?period=all", { credentials: "same-origin" })
  ).text();
  const doc = new DOMParser().parseFromString(html, "text/html");
  const social = new Set();
  doc.querySelectorAll('[data-testid="event-row"]').forEach((r) => {
    if (r.cells[1] && r.cells[1].textContent.trim() === "Social") {
      const a = r.querySelector('a[href^="/operate/events/"]');
      if (a) social.add(a.getAttribute("href").split("?")[0]);
    }
  });
  const eventOf = (node) => {
    const a = Array.from(node.querySelectorAll('a[href^="/operate/events/"]')).find((x) =>
      /^\/operate\/events\/[0-9a-f-]{36}/.test(x.getAttribute("href")),
    );
    return a ? a.getAttribute("href").match(/^\/operate\/events\/[0-9a-f-]{36}/)[0] : null;
  };
  const kept = { rows: 0, people: new Set(), events: new Set() };
  const main = document.querySelector("main");
  main.querySelectorAll("tbody tr").forEach((tr) => {
    const ev = eventOf(tr);
    if (ev === null) return;
    if (!social.has(ev)) tr.remove();
    else if (window.innerWidth >= 900) {
      kept.rows += 1;
      kept.events.add(ev);
      kept.people.add(tr.cells[1] ? tr.cells[1].textContent.trim() : String(kept.rows));
    }
  });
  /* Phone cards: any card-like block with one event link. */
  if (window.innerWidth < 900) {
    main.querySelectorAll('a[href^="/operate/events/"]').forEach((a) => {
      const m = a.getAttribute("href").match(/^\/operate\/events\/[0-9a-f-]{36}/);
      if (!m) return;
      let card = a.parentElement;
      while (
        card &&
        card !== main &&
        card.querySelectorAll('a[href^="/operate/events/"]').length <= 1 &&
        !card.classList.contains("MuiPaper-root")
      )
        card = card.parentElement;
      if (!card || card === main) return;
      if (!social.has(m[0])) card.remove();
      else {
        kept.events.add(m[0]);
        kept.rows += 1;
      }
  ...
  /* Empty groups: any heading block left with no rows. */
  main.querySelectorAll("table").forEach((t) => {
    if (t.tBodies[0] && t.tBodies[0].rows.length === 0)
      (t.closest(".MuiPaper-root") || t).style.display = "none";
  });
  const sub = main.querySelector(
    '[data-testid="page-header"] p, [data-testid="page-header"] .MuiTypography-body2',
  );
  if (sub && /\d/.test(sub.textContent))
    sub.textContent = sub.textContent
      .replace(/\d+(\s+\w+)?(\s*·\s*)\d+/, (m0) => m0)
      .replace(
        /(\d+)(\D+?)(\d+)(\D*)$/,
        (_m, _a, b, _c, d) => `${kept.people.size || kept.rows}${b}${kept.events.size}${d}`,
      );
})();
