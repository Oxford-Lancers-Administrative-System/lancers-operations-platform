/* W3-01 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W3-01 — the roster board as the Kit Manager after W1-03: View on Person,
   * Edit on Kit, None on the other nine categories, May add to the roster off.
   * Only Person's and Kit's columns exist; Add players is absent; a player's
   * name still opens the record (W3-03), because records open for anyone who
   * can reach the roster. Person's values read "view" and link nowhere.
   * Contactable and Missing stay in Person, where they are on main.
   *
   * Photographed as the review account (every grant) and narrowed by script;
   * sidebar and name are the Kit Manager's. Kit arrives folded for this
   * account (LAN-387): the script opens it, copies the board, and folds it
   * again, so the account's stored preference ends exactly as it was.
   */
  ...
  if (WIDE) {
    ...
    const { copy, shown: n } = narrowBoard(snapshot, {
      granted: ["person", "kit"],
      viewOnly: ["person"],
    });
    shown = n;
    await scrollBoardTo(copy, "band-toggle-kit", 480);
  }

  /* Phone: the Filters collapse held only filters this seat is not granted; cards lose the Membership status chip. */
  ...
  main.querySelectorAll('[data-testid="roster-card"]').forEach((card) => {
    card.querySelectorAll('[data-domain="membership"]').forEach((n) => n.remove());
  });

  const label = main.querySelector('[data-testid="season-label"]');
  /* The phone draws cards, not the table; it reads the count measured on the desktop board (6 Person + 12 Kit). */
