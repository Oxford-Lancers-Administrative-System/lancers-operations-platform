/* W1-03 — generated from the shared prelude, kit and this screen's body; see the notes inside. */
(async () => {
  /*
  ...
  /*
   * W1-03 — the Kit Manager's seat page, the moment after Kit changes from
   * None to Edit. Holder at the top as today; Access underneath, replacing the
   * Permissions list: Roster (eleven categories), Recruiting (three), Event
   * templates (one line per template on main, read from the running templates
   * page), Adding people (two switches), and the two mass edits marked
   * Proposed for owner approval. Every other seat starts with None, so the
   * Kit Manager's Person View a minute earlier and Kit Edit now are the only
   * grants. Each change is one audited write and one History entry.
   */
  await appButtons();
  accessSection({
    grants: seed("kit"),
